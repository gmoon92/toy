...
## 1. 실제 사례: 파트너 어드민 사이트의 504 타임아웃 이슈 (3분)
...
파트너 관리자 미결제 내역 페이지에서 504 타임아웃이 발생하여 페이지가 아예 로드되지 않는 현상이 있었습니다.
...
**시나리오**: 사용자 페이지에서 이름으로 검색하는 요구사항

```sql
-- 사용자 이름 검색 쿼리
SELECT * FROM tb_user WHERE name LIKE 'gmoon%';
```

### 2-2. EXPLAIN으로 문제 확인

```sql
EXPLAIN SELECT * FROM tb_user WHERE name LIKE 'gmoon%';
```
...
### 2-3. Full Scan이 느린 이유: Disk I/O

**Full Scan 동작 과정**
```
1. MySQL 엔진이 쿼리 실행
   ↓
2. tb_user 테이블의 모든 Data Page를 읽기 시작
   ↓
3. 100만 건의 데이터를 순차적으로 확인
   - 각 Row의 name 컬럼을 검사
   - 'gmoon'으로 시작하는지 확인
   ↓
4. 조건에 맞는 Row만 반환
...
100만 건의 데이터를 처음부터 끝까지 읽으면서 이름을 확인해야 하는 거죠. 당연히 느릴 수밖에 없습니다.
...
- `key: idx_name` → 인덱스 사용 중 ✅
- `rows: 4` → 100만 건 → 4건으로 대폭 감소 ✅

**성능 개선**
- **검색 시간: 5초 → 0.01초 (500배 개선)**
- **읽은 Page 수: 수천 개 → 수십 개**
...
1. MySQL 엔진이 쿼리 실행
   ↓
2. Buffer Pool에서 tb_user의 모든 Data Page 확인
   - Buffer Pool에 없는 Page는 Disk에서 로드 (Disk I/O 발생)
   ↓
3. 모든 Page를 순차적으로 읽으며 name 컬럼 확인
...
"인덱스를 추가하니 type이 range로 바뀌고, key에 idx_name이 표시됩니다. 그리고 rows가 100만에서 4로 줄었습니다. 성능은 500배 개선되었죠.

왜 이렇게 빠를까요?

인덱스가 없으면 MySQL은 테이블 전체를 스캔해야 합니다. 100만 건의 데이터가 저장된 수천 개의 Page를 모두 읽어야 하죠.

하지만 인덱스가 있으면 다릅니다. 먼저 인덱스를 탐색해서 'gmoon'으로 시작하는 데이터의 위치를 빠르게 찾습니다. 그리고 해당 위치의 데이터만 읽으면 되므로 극소수의 Page만 접근하면 됩니다.
...
**인덱스 검색 프로세스**
1. 인덱스(B+Tree)를 탐색하여 조건에 맞는 리프 노드 찾기
2. 리프 노드에서 PK(클러스터링 키) 추출
3. PK를 이용해 실제 테이블 데이터 페이지에서 Row 조회

**핵심 포인트**
- 리프 노드에 저장된 PK = 실제 테이블 Row의 위치
- PK로 실제 데이터를 가져올 때 **Random I/O 발생**
- Random I/O가 많을수록 성능 저하 → **인덱스 효율성의 핵심**

**발표 멘트**

"MySQL InnoDB는 B+Tree라는 자료구조로 인덱스를 구성합니다.

B+Tree는 세 가지 노드로 구성됩니다. 루트와 브랜치 노드는 키 값만 저장하여 탐색 경로를 제공하고, 리프 노드에는 실제 인덱스 값과 함께 PK가 저장됩니다.
...
인덱스 검색 과정을 보면, 먼저 B+Tree를 탐색해서 조건에 맞는 리프 노드를 찾습니다. 리프 노드에서 PK를 추출한 후, 이 PK를 이용해 실제 테이블 데이터 페이지에서 Row를 조회합니다.

이때 PK로 실제 데이터를 가져오는 과정에서 Random I/O가 발생합니다. Random I/O가 많을수록 성능이 떨어지기 때문에, 인덱스 설계 시 이를 최소화하는 것이 핵심입니다.

이제 단일 인덱스로는 해결할 수 없는 더 복잡한 케이스를 살펴보겠습니다."

## 4. 해결 방법 2: 복합 조건 검색 최적화 (7분)
...
### 4-5. 선택도(Selectivity) - 좋은 인덱스의 조건
...
## 5. 최적화 심화: 커버링 인덱스와 정렬 (4분)

### 5-1. 커버링 인덱스 (Covering Index) - 최고의 최적화
...
## 6. 옵티마이저 이해: 인덱스 선택의 주체 (6분)

### 6-1. 비용 기반 옵티마이저 (CBO) - 인덱스를 선택하는 주체
...
## 7. 실전 튜닝 사례와 핵심 체크리스트 (5분)

### 7-1. 실제 개선 사례: 관리자 페이지 48초 → 124ms 개선
