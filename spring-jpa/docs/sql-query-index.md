...
- 노드 당 많은 키: B-tree의 각 노드는 여러 개의 키를 포함할 수 있어, 한 번의 디스크 I/O로 많은 키를 읽을 수 있다. 이는 메모리 효율성을 높이고 디스크 접근 횟수를 줄인다.
...
![rule-based-optimization.png](images/rule-based-optimization.png)

### Query 실행 계획 우선 순위

옵티마이저의 쿼리 실행 계획의 우선 순위는 다음과 같다.
...
옵티마이저는 1~4 을 기준으로 index 활용할지 여부를 판단하고, 다음으로 5~7 실제 데이터 블록 접근한다.
