...
시간 순서로 정렬 가능한 고유 식별자를 생성하기 위한 Java 라이브러리로, Snowflake 와 ULID Spec 의 아이디어를 결합한 방식이다.
...
1. **42비트 타임스탬프**: 현재 시간을 밀리초 단위로 표현하여 시간 순서에 따라 정렬이 가능하다.
2. **22비트 랜덤 값**: 동일한 타임스탬프 내에서 고유성을 보장한다.
   - 노드 ID(0~20비트)
   - 카운터(2~22비트)

> 카운터 비트는 노드 비트에 따라 달라진다. 노드 비트가 `10`이면 카운터 비트는 `12`로 제한된다. 
> ex) 최대 노드 값은 1023(2^10-1), 카운터 값은 4095(2^12-1)다. 따라서 밀리초당 생성할 수 있는 최대 TSID는 4096다.

#### TSID 특징

- 시간 기반으로 정렬 가능하여 범위 검색과 데이터 정렬이 용이하다.
- 랜덤 값을 사용해 고유성을 보장하고 충돌을 방지한다.
- 64 비트로 Java Long 자료형으로 대체될 수 있다.
- 길이는 UUID, ULID, KSUID보다 짧다.

### TSID VS Snowflake ID

|         | TSID                    | Snowflake ID                 |
|---------|-------------------------|------------------------------|
| 타임스탬프   | 42비트 (Epoch 기준)         | 41비트 (Epoch 기준)              |
| 기기 ID   | 데이터 센터 ID 없음            | 10비트 (5비트 데이터 센터, 5비트 기기 ID) |
| 일련번호    | 22비트 랜덤 값               | 12비트 일련번호                    |
| ID 길이   | 총 64비트                  | 총 64비트                       |
