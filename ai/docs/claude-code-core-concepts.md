...
- 워크플로나 작업 단위가 명시된 문서도 의도한 대로 동작하지 않을 수 있음

> **주의**: `.md` 파일에 작성된 워크플로나 꼭 지켜야 할 지침들도 compact되면서 핵심 내용이 유실될 수 있습니다.
...
| **적합한 작업** | 단일 작업, API 호출 | 복잡한 워크플로, 도메인 지식이 필요한 작업 |
...
| `SessionEnd` | 세션 종료 시 |
| `UserPromptSubmit` | 프롬프트 제출 전 (차단 가능) |
| `PermissionRequest` | 도구 실행 권한 요청 전 (차단 가능) |
| `PreToolUse` | 도구 호출 실행 전 (차단 가능) |
| `PostToolUse` | 도구 성공 후 |
| `PostToolUseFailure` | 도구 실패 후 |
| `Stop` | Claude 응답 완료 시 (차단 가능) |
| `StopFailure` | Claude 응답 실패 시 |
| `PreCompact` | 컨텍스트 압축 전 |
| `PostCompact` | 컨텍스트 압축 후 (요약 수신) |
| `Notification` | 알림 발생 시 |
| `SubagentStart` | 서브에이전트 시작 시 |
| `SubagentStop` | 서브에이전트 종료 시 |

> 이 외에도 `Setup`, `TeammateIdle`, `TaskCreated`, `TaskCompleted`, `ConfigChange`, `WorktreeCreate`, `WorktreeRemove`, `InstructionsLoaded`, `CwdChanged`, `FileChanged` 등 다양한 이벤트가 지원됩니다. 전체 목록은 [claude-code-hooks.md](claude-code-hooks.md)를 참조하세요.
...
> **`prompt` / `agent` 타입 제약**: 이 두 타입은 `PreToolUse`, `PostToolUse`, `PermissionRequest` 이벤트에서만 사용할 수 있습니다. 다른 이벤트(예: `SessionStart`, `Stop`)에서는 `command` 또는 `http` 타입을 사용해야 합니다.
...
> `PostToolUse`, `Notification` 등 일부 이벤트는 차단을 지원하지 않으며, exit code 2를 반환해도 피드백으로만 처리됩니다.
...
지금까지 살펴본 CLAUDE.md, 스킬, 훅을 포함한 주요 문서 타입의 차이점을 비교합니다.
...
이제 이러한 개념들을 실제로 어떻게 활용할 수 있는지 살펴보겠습니다.
...
오케스트레이션의 한 예시로, Git Worktree를 활용한 병렬 작업 방법을 살펴보겠습니다.
