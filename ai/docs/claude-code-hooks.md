...
> 이외에도 Hook 을 이용한 다음 작업들을 자동화할 수 있습니다.
...
`Claude Code 알림 Hook 설정 예제`처럼 `settings.json` 설정 파일에 command를 지정하여 설정할 수 있습니다.

Claude Code CLI에서 `/hooks` 명령어로 대화형 메뉴를 열어 쉽게 설정할 수 있습니다.
...
Hook 이벤트는 Claude Code 라이프사이클의 특정 지점에서 발생합니다.
...
- **Agentic Loop** 
  ...
  - `ElicitationResult`: 사용자가 MCP elicitation에 응답한 후
- **응답 완료**
  - `Stop`: Claude 응답 완료 시
  - `StopFailure`: API 오류(rate limit, 인증 실패 등)로 턴이 종료될 때
...
라이프사이클을 이해했으니, 실제로 Hook을 어떻게 설정하는지 알아보겠습니다.
...
| `StopFailure`                                                          | API 오류 유형 | `rate_limit`, `authentication_failed`, `billing_error`, `invalid_request`, `server_error`, `max_output_tokens`, `unknown` |
| `Elicitation`, `ElicitationResult`                                     | MCP 서버 이름 | `mcp_server_name` 값                                                               |
...
> **참고**: `UserPromptSubmit`, `Stop`, `StopFailure`, `TeammateIdle`, `TaskCreated`, `TaskCompleted`, `WorktreeCreate`, `WorktreeRemove`, `CwdChanged`, `FileChanged`는 matcher를 지원하지 않습니다. (`FileChanged`는 파일명 패턴을 matcher로 사용합니다.)
...
| `asyncRewake`   | 아니오 | `true`면 백그라운드 실행하되 exit code 2 발생 시 모델을 깨워 처리 (async 포함) |
...
이벤트의 종류를 알았으니, 각 이벤트가 정확히 어떤 데이터를 주고받는지 살펴보겠습니다.
...
|----------------------|-------|------------------------------------|
| `PreToolUse`         | O     | 도구 호출 차단                           |
| `PermissionRequest`  | O     | 권한 거부                              |
| `UserPromptSubmit`   | O     | 프롬프트 처리 차단 및 삭제                    |
| `Stop`               | O     | Claude 중단 방지, 대화 계속                |
| `SubagentStop`       | O     | 서브에이전트 중단 방지                       |
| `TeammateIdle`       | O     | 팀원 유휴 상태 전환 방지                     |
| `TaskCreated`        | O     | 작업 생성 방지                           |
| `TaskCompleted`      | O     | 작업 완료 표시 방지                        |
| `Elicitation`        | O     | elicitation 거부 (action이 deny로 처리됨)  |
| `ElicitationResult`  | O     | 응답 차단 (action이 decline으로 처리됨)       |
| `ConfigChange`       | O     | 설정 변경 적용 차단 (`policy_settings` 제외) |
| `WorktreeCreate`     | O     | 생성 실패                              |
| `PostToolUse`        | X     | stderr를 Claude에게 표시 (이미 실행됨)       |
| `PostToolUseFailure` | X     | stderr를 Claude에게 표시                |
| `StopFailure`        | X     | fire-and-forget, 출력/exit code 무시   |
| 그 외                  | X     | stderr만 로그                         |
...
| `teammate_name`    | 작업을 생성한 팀원 이름 (optional) |
| `team_name`        | 팀 이름 (optional)          |

> **참고**: Exit code 2로 작업 생성을 차단할 수 있습니다.
...
|----------|---------------------------|
| `trigger` | `"manual"` 또는 `"auto"`   |
| `summary` | 압축된 컨텍스트 요약문              |

> **참고**: stdout은 사용자에게 표시됩니다.
...
MCP 서버가 사용자 입력을 요청할 때 실행됩니다.

**Matcher:** MCP 서버 이름 (`mcp_server_name`)

**추가 입력 필드:**

| 필드                 | 설명                      |
|--------------------|-------------------------|
| `mcp_server_name`  | elicitation을 요청한 MCP 서버 |
| `message`          | 서버가 사용자에게 전달하는 메시지      |
...
| 필드        | 설명                                          |
|-----------|---------------------------------------------|
| `action`  | `"accept"`, `"decline"`, `"cancel"` 중 하나   |
| `content` | `"accept"`일 때 반환할 콘텐츠 (optional)            |

> **참고**: exit code 2는 elicitation을 거부(deny)합니다.

### ElicitationResult

사용자가 MCP elicitation에 응답한 후 실행됩니다.
...
|------------------|--------------------------------------|
| `mcp_server_name` | elicitation을 요청한 MCP 서버              |
| `action`         | 사용자 응답 (`"accept"`, `"decline"` 등)  |
| `content`        | 사용자가 입력한 콘텐츠                         |
| `mode`           | 처리 모드                                |
| `elicitation_id` | elicitation 식별자                      |

> **참고**: exit code 2는 응답을 차단하여 action을 `decline`으로 처리합니다.

### CwdChanged

작업 디렉토리가 변경된 후 실행됩니다.

**추가 입력 필드:**

| 필드        | 설명              |
|-----------|-----------------|
| `old_cwd` | 이전 작업 디렉토리 경로   |
| `new_cwd` | 새 작업 디렉토리 경로    |

> **참고**: `CLAUDE_ENV_FILE`이 설정되어 있어 이후 Bash 명령에 적용할 환경 변수를 설정할 수 있습니다.

### FileChanged

감시 중인 파일이 변경될 때 실행됩니다.

**Matcher:** 감시할 파일명 패턴 (예: `.envrc|.env`)
...
|-------------|--------------------------------------|
| `file_path` | 변경된 파일 경로                            |
| `event`     | 이벤트 유형 (`change`, `add`, `unlink`)   |

기본 이벤트를 모두 살펴봤으니, 특수한 상황에서 사용하는 고급 기능을 알아보겠습니다.
...
셸 프로파일(`~/.zshrc` 또는 `~/.bashrc`)의 무조건적인 echo 문장이 JSON 앞에 추가될 수 있습니다:
