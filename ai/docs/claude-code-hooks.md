...
1. [개요](#1-개요)
2. [첫 번째 Hook 설정하기](#2-첫-번째-hook-설정하기)
3. [자동화 예제](#3-자동화-예제)
4. [Hook 이벤트 라이프사이클](#4-hook-이벤트-라이프사이클)
5. [Hook 설정 스키마](#5-hook-설정-스키마)
6. [입출력 및 제어 방식](#6-입출력-및-제어-방식)
7. [이벤트별 상세 참조](#7-이벤트별-상세-참조)
8. [고급 기능](#8-고급-기능)
9. [보안 및 문제 해결](#9-보안-및-문제-해결)

---

## 1. 개요

Hooks는 Claude Code의 라이프사이클 특정 시점에서 실행되는 사용자 정의 명령어입니다. 코드 포맷팅, 알림 전송, 명령어 검증, 프로젝트 규칙 강제 등의 작업을 자동화할 수 있습니다.
...
`/hooks` 명령어로 대화형 메뉴를 열어 쉽게 설정할 수 있습니다:
...
> **참고**: `jq` 설치 필요 (`brew install jq` 또는 `apt-get install jq`)

### 3. 보호된 파일 편집 차단

`.claude/hooks/protect-files.sh` 생성:
...
for pattern in "${PROTECTED_PATTERNS[@]}"; do
  if [[ "$FILE_PATH" == *"$pattern"* ]]; then
    echo "차단됨: $FILE_PATH matches protected pattern '$pattern'" >&2
    exit 2
  fi
done
...
예제를 통해 알아본 Hook의 동작을 더 깊이 이해하기 위해, Hook이 어떤 순서로 실행되는지 살펴 보겠습니다.
...
| 단계               | 이벤트                  | 설명                 |
|------------------|----------------------|--------------------|
| **세션 관리**        | `SessionStart`       | 세션 시작/재개 시         |
|                  | `UserPromptSubmit`   | 프롬프트 제출 전          |
| **Agentic Loop** | `PreToolUse`         | 도구 호출 실행 전 (차단 가능) |
|                  | `PermissionRequest`  | 권한 대화 상자 표시 시      |
|                  | `[Tool Execution]`   | 도구 실제 실행           |
|                  | `PostToolUse`        | 도구 성공 후            |
|                  | `PostToolUseFailure` | 도구 실패 후            |
|                  | `TaskCompleted`      | 작업 완료 시            |
| **응답 완료**        | `Stop`               | Claude 응답 완료 시     |
|                  | `TeammateIdle`       | 팀원 유휴 상태 전환 시      |
|                  | `PreCompact`         | 컨텍스트 압축 전          |
|                  | `SessionEnd`         | 세션 종료 시            |
| **비동기/설정**       | `Notification`       | 알림 발생 시            |
...
설정 파일에 직접 편집한 내용은 즉시 적용되지 않습니다. Claude Code는 세션 시작 시 hooks의 스냅샷을 캡처하여 세션 내내 사용합니다. 외부에서 hooks가 수정되면 `/hooks` 메뉴에서 검토를
요구합니다.
