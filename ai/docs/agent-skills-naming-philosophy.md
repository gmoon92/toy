...
- 잘못된 예 — 도구: "PDF 처리기" (객체)
- 올바른 예 — 지침: "PDF를 처리하는 방법" (행위)

Claude가 스킬 이름을 해석하는 방식은 다음과 같습니다.

| 패턴        | 예시                | Claude의 해석                   | 의미 전달 | 스킬 본질과의 일치  |
|-----------|-------------------|------------------------------|-------|-------------|
| **동명사**   | `processing-pdfs` | "PDF를 처리하는 **행위/활동**에 대한 지침" | 5/5 | O 행위 중심 지침  |
| **명사구**   | `pdf-processing`  | "PDF 처리" (중립적)               | 3/5 | - 중립적       |
| **명사+er** | `pdf-processor`   | "PDF 처리기" (도구/객체)            | 2/5 | X 객체로 오해 가능 |
| **동사 원형** | `process-pdf`     | "PDF를 처리하라" (명령)             | 3/5 | - 명령형       |
...
게임의 '힐링'이 "천천히 체력을 회복하는 행위"를 즉각 연상시키듯, 동명사는 행위의 진행성과 결과물을 동시에 암시합니다.
...
| 이름                       | Claude가 읽는 방식               |
|--------------------------|-----------------------------|
| `analyzing-spreadsheets` | "스프레드시트를 분석하는 활동에 대한 가이드" (적합) |
