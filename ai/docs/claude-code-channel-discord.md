...
Discord 봇을 통해 Claude Code 세션과 실시간으로 메시지를 주고받는 양방향 채널을 설정하는 방법을 알아봅니다.
...
- 인증 요구사항
  ...
  - **팀/엔터프라이즈 조직**: 관리자가 [claude.ai Admin Console](https://claude.ai/admin-settings/claude-code)에서 채널 기능을 활성화해야 함
...
**플러그인 마켓플레이스 등록**
```bash
# 공식 마켓플레이스가 아직 등록되어 있지 않다면 먼저 추가
...
> 마켓플레이스를 찾을 수 없다는 오류가 나면 마켓플레이스를 먼저 등록한 후 플러그인을 다시 설치하세요.
> ```bash
> claude plugin marketplace add anthropics/claude-plugins-official
> ```
...
> 토큰은 `~/.claude/channels/discord/.env`에 자동으로 저장됩니다. 환경 변수로 설정할 수도 있습니다:
> ```bash
...
1. Discord에서 봇에게 DM을 보내면 봇이 페어링 코드를 보내줍니다
...
2. Claude Code에서 코드를 입력합니다:
   ...
   ```
3. 본인 계정만 메시지를 전달하도록 허용 목록 정책을 활성화합니다:
   ```
...
- [Claude Code 공식 문서 - Channels Reference](https://code.claude.com/docs/en/channels-reference)
- [Claude Code 공식 문서 - Supported Channels](https://code.claude.com/docs/en/channels#supported-channels)
