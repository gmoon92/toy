...
최신 Claude 모델에서는 프롬프트 토큰과 출력 토큰의 합이 모델의 컨텍스트 윈도우를 초과하면 **시스템이 컨텍스트를 조용히 자르는 대신 `model_context_window_exceeded` stop_reason을 반환**합니다. (extended-context-window 베타 미사용 시에는 400 에러로 반환됩니다.)

이 동작은 더 예측 가능한 결과를 제공하지만 더 신중한 토큰 관리가 필요합니다.
...
Claude Sonnet 4 계열(4, 4.5, 4.6) 및 Claude Opus 4.6은 **100만 토큰** 컨텍스트 윈도우를 지원합니다.

| 항목 | 내용 |
|-----|------|
| **지원 모델** | Claude Sonnet 4, Claude Sonnet 4.5, Claude Sonnet 4.6, Claude Opus 4.6 |
...
| **클라이언트 측 압축** | SDK에서 요약 생성 후 전체 기록 대체 |
...
> **주의:** 여러 전략을 사용할 때 `clear_thinking_20251015` 전략은 `edits` 배열에서 먼저 나열되어야 합니다.
...
- **Thinking 블록 지우기**: thinking 블록이 컨텍스트에 유지되면 프롬프트 캐시가 보존되어 캐시 히트를 활성화하고 입력 토큰 비용을 줄입니다.
...
5. **보존할 컨텍스트**: 사용자 선호 사항, 도메인별 세부 정보, 핵심 약속
...
컨텍스트 편집 기능을 활용하면 더 효율적으로 토큰을 관리할 수 있습니다.
...
| 모델 | 컨텍스트 윈도우 | 특징 |
|-----|---------------|------|
| Claude Opus 4.6 | 200K / 1M (베타) | 최고 성능 |
| Claude Opus 4.5 | 200K | 고성능 |
| Claude Opus 4.1 | 200K | 고성능 추론 |
| Claude Opus 4 | 200K | 복잡한 작업에 최적 |
| Claude Sonnet 4.6 | 200K / 1M (베타) | 장문 컨텍스트 지원 |
| Claude Sonnet 4.5 | 200K / 1M (베타) | 장문 컨텍스트 지원 |
| Claude Sonnet 4 | 200K / 1M (베타) | 균형 잡힌 성능 |
| Claude Haiku 4.5 | 200K | 경량 모델 |
