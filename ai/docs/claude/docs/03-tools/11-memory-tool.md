...
Memory 도구는 Claude가 메모리 파일 디렉토리를 통해 대화 간에 정보를 저장하고 검색할 수 있도록 합니다. 
Claude는 세션 간에 지속되는 파일을 생성, 읽기, 업데이트 및 삭제할 수 있으므로, 모든 것을 컨텍스트 윈도우에 유지하지 않고도 시간이 지남에 따라 지식을 구축할 수 있습니다.
...
활성화되면 Claude는 작업을 시작하기 전에 자동으로 메모리 디렉토리를 확인합니다. 
Claude는 작업하는 동안 학습한 내용을 저장하기 위해 `/memories` 디렉토리에서 파일을 생성, 읽기, 업데이트 및 삭제할 수 있으며, 나중에 해당 메모리를 참조하여 유사한 작업을 더 효과적으로 처리하거나 중단된 지점부터 재개할 수 있습니다.

이것은 클라이언트 측 도구이므로 Claude는 메모리 작업을 수행하기 위해 도구 호출을 하고, 애플리케이션은 해당 작업을 로컬에서 실행합니다. 이를 통해 메모리가 저장되는 위치와 방법을 완전히 제어할 수 있습니다.
보안을 위해 모든 메모리 작업을 `/memories` 디렉토리로 제한해야 합니다.
...
> 애플리케이션에서 memory 도구 작업을 처리하려면 각 memory 명령에 대한 핸들러를 구현해야 합니다. 우리의 SDK는 도구 인터페이스를 처리하는 memory 도구 헬퍼를 제공합니다.
`BetaAbstractMemoryTool`(Python)을 서브클래스화하거나 `betaMemoryTool`(TypeScript)을 사용하여 자체 메모리 백엔드(파일 기반, 데이터베이스, 클라우드 스토리지, 암호화된
> 파일 등)를 구현할 수 있습니다.
>
> 작동하는 예제는 다음을 참조하세요:
> -
...
- **중복 텍스트**: `old_str`이 여러 번 나타나는 경우:
  ``"No replacement was performed. Multiple occurrences of old_str `{old_str}` in lines: {line_numbers}. Please ensure it is unique"``
...
> 참고: 메모리 폴더를 편집할 때는 항상 내용을 최신 상태로 유지하고 일관성 있고 체계적으로 관리하세요. 더 이상 관련이 없는 파일은 이름을 변경하거나 삭제할 수 있습니다. 필요하지 않으면 새 파일을 생성하지
> 마세요.
...
메모리 파일 크기를 추적하고 파일이 너무 커지는 것을 방지하는 것을 고려하세요. 
memory read 명령이 반환할 수 있는 최대 문자 수를 추가하고 Claude가 내용을 페이지별로 읽을 수 있도록 하는 것을 고려하세요.
...
Memory 도구는 [text editor 도구](../03-tools/08-text-editor-tool.md)와 유사한 오류 처리 패턴을 사용합니다. 
자세한 오류 메시지 및 동작은 위의 개별 도구 명령 섹션을 참조하세요. 
일반적인 오류로는 파일을 찾을 수 없음, 권한 오류, 잘못된 경로 및 중복 텍스트 일치가 있습니다.

## Context Editing과 함께 사용

Memory 도구는 [context editing](../02-capabilities/02-context-editing.md)과 결합할 수 있습니다. 
context editing은 대화 컨텍스트가 구성된 임계값을 초과하면 오래된 도구 결과를 자동으로 지웁니다. 
이 조합은 그렇지 않으면 컨텍스트 제한을 초과할 장기 실행 에이전트 워크플로우를 가능하게 합니다.

### 함께 작동하는 방식

Context editing이 활성화되고 대화가 지우기 임계값에 접근하면 Claude는 자동으로 경고 알림을 받습니다. 
이것은 Claude가 도구 결과가 컨텍스트 윈도우에서 지워지기 전에 도구 결과의 중요한 정보를 메모리 파일에 보존하도록 유도합니다.

도구 결과가 지워진 후 Claude는 필요할 때마다 메모리 파일에서 저장된 정보를 검색할 수 있으며, 메모리를 작업 컨텍스트의 확장으로 효과적으로 처리합니다. 
