...
sequenceDiagram
    ...
    participant S as 서버
    C->>S: 로그인
    Note over S: 세션 저장 { sid-123 → {user: 홍길동, role: admin} }
    S-->>C: sid-123 (쿠키)
    C->>S: 요청 + sid-123
    Note over S: 매 요청마다 저장소에서 sid-123 조회해 신원 확인
```
...
- 서버는 상태를 갖지 않으므로 인스턴스를 마음껏 늘릴 수 있다(수평 확장).
- 저장소 조회가 없으니 검증이 빠르다.
- 다만 새 문제가 생긴다. 정보가 클라이언트 손에 있으니 위조·변조를 어떻게 막을 것인가? (홍길동이 자기 토큰의 `role: user`를 `role: admin`으로 고친다면?)
...
| 서명 방식 | 원리 | 한계 / 이점 |
|---|---|---|
| **대칭키(HMAC, 예: HS256)** | 발급자와 검증자가 **같은 비밀키**를 공유해 서명·검증 | 한계. 검증자가 많아질수록 비밀키가 여기저기 복제되어 유출 위험이 커지고, 검증자도 그 키로 똑같이 위조할 수 있어 발급자와 검증자가 구분되지 않는다. |
| **비대칭키(예: ES256/RS256)** | 발급자만 **개인키**로 서명, 검증자는 **공개키**로 검증만 | 이점. 검증자는 개인키가 없으니 위조할 수 없고, 공개키는 공개돼도 안전해 얼마든지 나눠줄 수 있다. 발급자 하나에 검증자 여럿인 구조에 잘 맞는다. |

핵심은 여기에 있습니다. 비대칭 서명 덕분에 발급은 개인키를 쥔 한 곳에서, 검증은 공개키만 가진 여러 곳에서 이뤄집니다. 검증자는 발급자에게 매번 이 토큰이 진짜냐고 물을 필요 없이, 손에 든 공개키로 그 자리에서 무상태로 검증합니다. 세션 방식이 하지 못하던 일을 바로 이 구조가 해결합니다.
...
```mermaid
flowchart LR
    A["JWS 헤더의 kid<br/>이 토큰은 kid=A로 서명했다"]
    B["JWKS 안 어떤 JWK의 kid<br/>kid=A 공개키는 이것이다"]
    A -->|" 일치 "| B
```
...
> 정리하면 발전 과정은 이렇게 이어집니다. 세션으로 서버가 상태를 보관하다가 확장·공유·조회 비용의 한계에 부딪혀, 토큰에 정보를 담기로 합니다(JWT). 그러자 위조 문제가 생겨 서명을 붙이고(JWS), 검증자를 믿기 어렵고 그 수도 많으니 비대칭키로 넘어갑니다. 공개키를 배포하려고 JWK와 JWKS를 두고, 키 교체를 식별하려고 `kid`와 인증 서버의 키 관리를 더합니다. 마지막으로 내용까지 숨겨야 할 때 JWE로 갑니다.
...
왜 굳이 인코딩을 거칠까요. HTTP 헤더와 URL에 안전하게 싣기 위해서입니다. JSON 원문에는 `{`, `}`, `"`, 공백, 줄바꿈처럼 헤더나 쿼리 파라미터에서 깨지거나 다르게 해석되는 문자가 많습니다. 토큰은 `Authorization` 헤더나 쿼리스트링에 실려야 하므로, 어느 맥락에 넣어도 안전한 문자 집합(`A–Z a–z 0–9 - _`)으로 먼저 변환해 두는 것입니다.

> 주의. Base64Url은 인코딩일 뿐 암호화가 아닙니다. Payload가 `eyJ...`로 보여 암호화된 것 같지만, 누구나 즉시 디코딩해 읽을 수 있습니다.

다만 JWK/JWKS(§4)는 헤더가 아니라 응답 바디(JSON)로 오가므로 인코딩이 필요 없고, 평문 그대로 전송됩니다.
...
그렇다면 누가 관리할까요. 이 모든 키의 생애주기, 곧 생성과 `kid` 부여, JWKS 게시, 회전, 폐기는 발급한 인증 서버가 단일 주체로 관리합니다. 개인키(`d`)는 인증 서버 밖으로 절대 나가지 않고, 공개키만 JWK로 JWKS에 실려 검증자에게 배포됩니다. 검증자는 키를 만들지도 소유하지도 않으며, 받아서 캐시해 두고 `kid`로 골라 쓸 뿐입니다.

JWK와 JWKS는 인코딩 없이 평문 JSON 그대로 전송됩니다(§3 표와 대비). JWS나 JWE는 `eyJ...` 문자열 덩어리로 변환되지만 JWK/JWKS는 예외인데, 공개키는 애초에 공개돼도 되는 정보라 숨길 이유가 없기 때문입니다.
...
`kid`가 놓는 다리를 다시 정리하면 이렇습니다. 헤더의 `kid`(파라미터)에서 JWKS 안의 같은 `kid`를 가진 JWK(공개키)로, 다시 그 공개키로 서명을 검증합니다. 이 한 줄이 "헤더의 `kid`가 발급한 인증 서버가 관리하는 키를 가리킨다"의 전부입니다. 거듭 강조하지만 `kid`는 Payload 클레임이 아니라 헤더 파라미터입니다.
...
왜 두 방식을 섞을까요. 비대칭 암호화는 느려서 큰 데이터에 부적합하고, 대칭 암호화는 빠르지만 키 공유가 어렵습니다. JWE는 둘의 장점만 취합니다. 본문은 임의로 생성한 대칭키(CEK)로 빠르게 암호화하고(단 ④), 그 대칭키를 수신자의 공개키(비대칭)로 안전하게 감싸 전달합니다(단 ②). 수신자는 자신의 개인키로 대칭키를 풀어 낸 뒤, 그 대칭키로 본문을 복호화합니다.
...
- 신·구 `kid`가 공존하므로 키를 무중단으로 교체할 수 있습니다.

이 흐름을 DPoP 소유 증명으로 한 단계 강화하면, 곧 토큰에 클라이언트 공개키 지문 `cnf`를 박아 두면 "토큰을 탈취해도 개인키가 없으면 쓰지 못한다"가 됩니다. 그 설계가 [DPoP 기반 실시간 릴레이 인증 아키텍처](sender-constrained-relay-architecture.md)입니다.
