...
| **JWK** (JSON Web Key) | 키 표현 | 공개키·개인키를 표현하는 표준 포맷(`{kty,crv,x,y}`). 인가 서버 서명키와 클라이언트 임시키가 모두 이 형태로 오갑니다. |
...
- **임시 키쌍(Ephemeral Key Pair)** — 클라이언트가 세션 단위로 EC P-256 임시 키쌍을 생성합니다. 개인키는 프로세스 메모리(또는 보안 저장소)에만 존재하고 반출되지 않습니다. (비유의 "손가락")
...
| 구분 | **키쌍 A — 인가 서버 서명키** | **키쌍 B — 클라이언트 임시키** |
|------|------------------------------|-------------------------------|
| 소유자 | 인가 서버 | 클라이언트(뷰어·에이전트) |
| 생성 시점 | 사전 생성, 서명키 저장소(DB)에서 관리(`id`=kid) | 세션마다 런타임 생성 |
| 용도 | Access Token(JWS) 서명·발급 | nonce 챌린지 응답 서명 |
| 공개키 전달 | JWKS(`GET /jwk`)로 검증 서버가 캐싱 | 토큰 발급 요청 시 인가 서버로 → `cnf`에 바인딩 |
...
`kid` 캐시 미스에 따른 JWKS 재요청은 오직 키쌍 A에만 해당합니다. 키쌍 B는 토큰 내부의 `cnf` 공개키로만 검증하므로 외부 질의가 없습니다.
...
sequenceDiagram
    ...
    Note over C, S: [세션 발급] HTTPS / WSS 통신
    C ->> C: 임시 키쌍 생성<br/>(priv 은닉, pub=JWK)
    C ->> S: JWT 요청 + 공개키(JWK) + 대상 식별자(target_device_id)<br/>(RoomID·UserID는 클라가 보내지 않음)
    S ->> S: 세션에서 user_id 추출 + 권한 검증 후 space_id(RoomID) 서버가 결정/생성<br/>cnf=클라 공개키 바인딩 + space_id·user_id·type 클레임<br/>서명키(kid=2)로 JWS 서명·발급
    S -->> C: JWS (cnf · space_id · type 포함)
...
### [Step 1] 클라이언트 임시 키쌍 생성 (개인키 은닉)

클라이언트(뷰어·호스트)가 Relay 정보를 요청하기 **전에** EC P-256 키쌍을 생성합니다. 개인키는 기기 메모리 밖으로 나가지 않으며, 공개키만 JWK 형태로 다음 단계에 실립니다.
...
| 클라이언트 유형 | 개인키 저장 | 확보 방법 | 비용·주의 |
|---|---|---|---|
| **에이전트(디바이스)** | OS 키스토어(비추출) | 기기 등록 시 만든 키쌍 재사용 가능 | ~0 (이미 있으면) |
| **네이티브 앱 뷰어** | Android Keystore / iOS Keychain | 앱 최초 실행 시 키쌍 생성·공개키 등록 | 낮음 |
| **웹 뷰어(브라우저)** | WebCrypto **non-extractable** `CryptoKey`(P-256) | 로그인/세션 요청 시 생성·공개키 등록 | 중 — 탭·세션·새로고침 시 키 수명 관리 필요 |
...
### [Step 2] 인가 서버에 JWT 요청 (공개키 + 대상 식별자 전달)

클라이언트는 **방금 만든 공개키(JWK)와 접근하려는 대상 식별자(`target_device_id` 등)만** 인가 서버로 전송해 Access Token을 요청합니다.

> **`room_id`·`user_id`는 클라이언트 입력으로 받지 않습니다 (IDOR 방지).** JWS 페이로드는 중계 서버가 읽을 '출력값'이지 클라이언트가 통제하는 '입력값'이 아닙니다. 만약 클라이언트가 `room_id`를 직접 지정하도록 두면, 공격자가 타인의 `room_id`를 넣어 유효한 토큰을 발급받는 IDOR 취약점이 생깁니다. 따라서 `user_id`는 인가 서버가 **로그인 세션 컨텍스트에서 직접 추출**하고, `room_id`(→`space_id`)는 인가 서버가 **권한 검증을 거쳐 직접 결정·생성**합니다([Step 3]).

<details>
<summary><b>💻 인가 서버 JWT 요청 (HTTP)</b></summary>

```http
POST /api/remote/relay-token HTTP/1.1
...

{
  ...
  "target_device_id": "dev-3f9c1a…",  // 접근하려는 대상 기기 식별자 (뷰어=방 참여 시)
  "pub_key": {                          // 클라이언트 임시 공개키 (cnf에 바인딩될 값)
    "kty": "EC", "crv": "P-256",
...
| 주체 | 클라 요청 | 인가 서버의 `room_id` 결정 | 발급 `type` |
|------|-----------|---------------------------|-------------|
| **호스트/에이전트 (방 개설)** | 공개키 + (기기 컨텍스트) | **새 RoomID(UUID)를 신규 생성**하고 저장소에 기록 | `agent` |
| **뷰어 (방 참여)** | 공개키 + `target_device_id` | 대상에 대한 **제어 권한을 검증**한 뒤 매핑된 기존 RoomID 조회 | `viewer` |

> **중계 서버의 연결·검증 시퀀스([Step 4]~[Step 5])는 호스트·뷰어가 100% 동일합니다.** 분기는 오직 인가 서버의 발급 비즈니스 로직에만 존재하며, 그 결과물인 JWS(cnf·space_id·type)를 검증하는 방식은 두 주체가 완전히 같습니다. 즉 호스트 에이전트가 방을 최초 개설하려 접속하는 순간에도 동일한 PoP 챌린지-리스폰스를 거칩니다 — 다만 그 시점의 `space_id`는 서버가 방금 생성한 새 RoomID입니다.
...
- [ ] **입력 통제 (IDOR 방지)** — 클라이언트로부터 `room_id`·`user_id`를 입력으로 받지 않는다. `user_id`는 로그인 세션에서 추출하고, `room_id`(→`space_id`)는 대상 식별자(`target_device_id`)에 대한 권한 검증 후 서버가 결정·생성한다.
...
```text
// 인가 서버 pseudo
{ targetDeviceId, pubJwk } = request.body     // 클라 입력: 대상 식별자 + 공개키만
...
- [ ] **임시 키쌍 사전 생성** — Relay 요청 전 EC P-256 키쌍 생성. **대원칙은 개인키의 외부 반출 금지**이며, 이를 만족하는 저장 방식(브라우저 `CryptoKey extractable:false`, 네이티브 Keychain·Keystore 등)은 플랫폼 전문가 재량으로 선택합니다. (로그·전송·디스크 평문 저장은 반출에 해당하므로 피합니다.)
- [ ] **JWT 요청** — 인가 서버에 `RoomID`·`UserID`·`pub_key`를 전달해 Access Token을 요청.
- [ ] **Relay 챌린지 응답(Sign)** — 받은 `nonce`를 `priv_key`로 서명해 회신. 매 연결마다 새 nonce에 서명하므로 과거 응답을 재사용할 수 없다.

<details>
<summary><b>💻 클라이언트 흐름 (pseudo)</b></summary>

```text
// Client pseudo (Viewer·Agent 공통)
keyPair = generateKeyPair("EC", "P-256")              // ① Relay 요청 전 키쌍 생성
jwt     = requestJwt(authServer, {                    // ② BE에 JWT 요청 (로그인 세션 위에서 호출)
            targetDeviceId, pubKey: keyPair.public })  //    대상 식별자 + 공개키만 전달 (room_id·user_id는 안 보냄)
...
스택별로 Viewer(JS)는 WebCrypto `subtle.generateKey`/`sign`, Agent(네이티브)는 플랫폼 crypto + OS 보안 저장소를 사용합니다.
...
| **P1-a · 근본** | 중계 서버 JWS 검증 신설 + 발신자 구속 전환 | 검증 게이트를 세우고 클라 임시키 + `cnf` 바인딩로 PoP 격상. 순수 TCP이므로 연결 최초 프레임에 토큰을 담아 무상태 검증. RoomID를 어떤 경로로 얻어도 개인키 없이는 접속 거부. |
