# OpenID Connect(OIDC) 아키텍처 완전 정복

> OAuth 2.0의 "인가"에서 출발해 "인증"의 표준까지 — 배경·개념·플로우·검증·위협·실무 아키텍처를 한 문서에서

---

## 이 문서에 대하여

이 문서는 인증·인가 아키텍처가 막막한 개발자를 **시니어 수준의 판단력**까지 끌어올리는 것을 목표로 합니다. 단순히 "이렇게 설정하세요"가 아니라, 각 설계가 **왜 그렇게 되었는가(why)**, 어떤 위협을 막고 어떤 트레이드오프를 감수하는가를 원리 수준에서 설명합니다.

**대상 독자**: OAuth/OIDC를 실무에 적용해야 하지만 개념이 뒤섞여 있는 백엔드·프론트엔드 개발자, 인증 아키텍처를 리뷰해야 하는 리드.

**함께 보면 좋은 문서**: 토큰 탈취를 근본적으로 무력화하는 sender-constrained token은 별도 문서 [`dpop-pop-architecture.md`](dpop-pop-architecture.md)에서 상세히 다룹니다. 이 문서에서는 5부에서 연결점만 짚습니다.
...
- [6부. 실무 아키텍처와 안티패턴 — 시니어의 관점](#6부-실무-아키텍처와-안티패턴--시니어의-관점)
- [종합 정리 — 시니어 코드 리뷰 체크포인트](#종합-정리--시니어-코드-리뷰-체크포인트)
- [참고 자료](#참고-자료)
...
OIDC는 OAuth의 **하위 확장이 아니라 형제(병렬 확장)**입니다. sender-constrained token(DPoP/mTLS) 역시 OAuth의 별도 확장으로, OIDC 흐름과 **결합**해 쓸 수 있습니다(→ 5부, DPoP 문서).
...
**두 가지 안티패턴을 반드시 기억하세요.**
- ID Token을 자원 서버(API)에 Bearer로 보내지 말 것 — 청중이 RP이지 API가 아님.
- Access Token으로 로그인을 구현하지 말 것 — §1.3의 의사 인증. 로그인엔 검증된 ID Token의 `sub`를 사용.
...
### `nonce` · `code_challenge` — 이름은 비슷하지만 생성 주체가 다르다

"챌린지-응답"은 이 생태계가 발명한 게 아니라 오래된 범용 개념입니다. 그런데 이름이 비슷한 세 값(`nonce`, PKCE `code_challenge`, DPoP `nonce`)은 **누가 값을 만드느냐**가 달라 성격이 갈립니다. 이 구분을 놓치면 "PKCE가 있으니 nonce는 불필요" 같은 오해가 생깁니다.
...
> **핵심**: OIDC `nonce`와 PKCE `code_challenge`는 이름과 달리 **클라이언트가 만든 값**(자가 커밋/반향)이라, ID Token/인가코드를 *내 세션에* 결속하는 용도입니다. 반면 **서버가 값을 던지는 진짜 챌린지-응답**은 이 생태계에서 **DPoP의 서버 nonce**가 대표적입니다(→ 별도 문서 [DPoP 아키텍처](dpop-pop-architecture.md) 4.4). 검증 대상·시점이 다르므로 PKCE가 있어도 `nonce`는 여전히 필요합니다.
...
OIDC 로그아웃은 **1개의 개시 스펙 + 3개의 통지 스펙**으로 나뉩니다.
...
## 5.4 Sender-Constrained Token 연계

지금까지의 방어는 **로그인 흐름의 무결성**을 다뤘습니다. 그러나 발급된 access token이 **베어러 토큰(bearer token)**이면 소지자는 누구든 정당 클라이언트와 구별되지 않습니다(SPA 유출·악성 확장·로그·TLS 종단 프록시).

해법은 **Sender-Constrained Token** — 토큰을 클라이언트 키에 바인딩해 탈취만으로는 못 쓰게 만드는 것입니다.
...
## 6.3 흔한 안티패턴과 오해 (매우 중요)

| 안티패턴 | 왜 문제인가 | 올바른 방식 |
| --- | --- | --- |
| **ID Token으로 API 접근** | aud 위반, scope 부재, 타 클라이언트 토큰 재사용 가능 | API에는 **Access Token** |
| **Access Token 무검증 신뢰** | 위조·재사용 토큰 통과 | 서명·iss·aud·exp·scope 검증 |
| **ID Token을 API에 Bearer 전송** | 수신자 오류, 인가 근거 없음 | Access Token만 전송 |
...
# 종합 정리 — 시니어 코드 리뷰 체크포인트

시니어의 인증 설계는 세 질문으로 압축됩니다: **이 클라이언트는 시크릿을 지킬 수 있는가**(컨피덴셜/퍼블릭), **토큰이 새면 무슨 일이 벌어지는가**(저장·폐기), **이 토큰은 원래 누가 읽으라고 만든 것인가**(ID/Access 구분).
...
*관련 문서: [DPoP / PoP 정공법 아키텍처](dpop-pop-architecture.md) — 토큰 탈취를 근본적으로 무력화하는 sender-constrained token의 상세 메커니즘.*
