---
title: DPoP 기반 실시간 릴레이 인증 아키텍처
description: 비(非)HTTP 실시간 채널(MQTT·WebSocket·TCP)에서 무상태 JWS 검증 구조를 DPoP 소유 증명으로 강화하는 토큰 보안 설계 가이드.
---

# DPoP 기반 실시간 릴레이 인증 아키텍처

> 비(非)HTTP 실시간 채널(MQTT·WebSocket·TCP)에서 **무상태 JWS 검증**을 유지하면서, **DPoP 소유 증명(RFC 9449)**으로 토큰 탈취·리플레이를 무력화하는 설계 가이드

## 목적

1. **발급 서버가 서명한 JWS를 검증자가 무상태로 검증**하는 단방향 비대칭키 구조에서, 토큰 탈취·리플레이 공격을 방어하기 위한 아키텍처 개선안(단말 바인딩 · DPoP)을 비교 분석한다.
2. 그 전제가 되는 **토큰 생태계 표준 기술**(JWT·JWS·JWE·JWK·Base64Url)을 실제 통신 예시와 함께 정의해, 실무자가 즉시 참고할 수 있게 한다.
3. DPoP는 원래 HTTP 헤더 스펙이지만, 이 문서는 그것을 **HTTP가 아닌 실시간 릴레이 채널에 응용**하는 방식을 다룬다. 특히 **데이터 중계 구간은 순수 TCP(헤더 없음)**이므로, `Authorization`/`DPoP` HTTP 헤더 대신 **연결 핸드셰이크 프레임(바이트 스트림)**에 토큰·Proof를 실어야 한다. (시그널링이 WebSocket인 경우에 한해 최초 upgrade 핸드셰이크에서 HTTP 헤더 사용 가능)

## ToC

**Part I. 토큰 생태계 표준 개념**

1. JWT·JWS·JWE·JWK 한눈에
2. JWT - 데이터 구조와 형식 (Base64Url 포함)
3. JWK - 키 표현 규격
4. JWS - 서명(변조 방지)
5. JWE - 암호화(기밀성)

**Part II. 현재 아키텍처**

6. 현재 구조와 그 이유
7. TLS 인증서 vs JWK
8. 문제점 분석

**Part III. 토큰 탈취 방어 개선안**

9. 리플레이 공격 모델
10. 방안 A - 단말 정보 바인딩
11. 방안 B - DPoP
12. 두 방안 비교

**Part IV. 설계 검토 및 결론**

13. 키 롤링 + 배포 전략
14. 개선 과제 (Action Items)
15. 최종 결론

---

# Part I · 토큰 생태계 표준 개념

## 1. JWT · JWS · JWE · JWK

먼저 용어 정리부터 하겠습니다.

**JWT는 "정보 전달 형식"이라는 상위 개념**이고,
JWS와 JWE는 그 JWT를 실제로 만드는 두 가지 구현 방식이며, JWK는 서명·암호화에 쓰는 **키 자체의 표현 규격**입니다.

| 용어       | 정식 명칭               | 역할                            | 보장하는 것   | 비유      |
|----------|---------------------|-------------------------------|----------|---------|
| **JWT**  | JSON Web Token      | 정보 전달 표준 문자열 (JWS 또는 JWE로 구현) | -        | 택배 상자   |
| **JWS**  | JSON Web Signature  | 내용은 **공개**, 서명으로 위변조만 방지      | 무결성 · 진위 | 봉인 스티커  |
| **JWE**  | JSON Web Encryption | Payload 자체를 **암호화**           | 기밀성      | 금고 상자   |
| **JWK**  | JSON Web Key        | 키를 JSON으로 표현하는 규격             | -        | 열쇠 그 자체 |
| **JWKS** | JWK Set             | 공개키 JWK 여러 개의 묶음 (키 롤링용)      | -        | 열쇠 꾸러미  |

> **흔한 오해:** "JWT = JWS"가 아닙니다. JWT라는 개념이 먼저 있고, 그것을 *서명만 하면* JWS, *암호화하면* JWE입니다. 이 아키텍처가 쓰는 것은 정확히는 **JWS**(서명만, 내용은 평문 공개)입니다.

## 2. JWT - 정보 전달의 "그릇": 데이터 구조와 형식

§1에서 봤듯 JWT은 그 자체로 서명이나 암호화를 하지 않습니다. 클레임(정보)을 담아 **어디에 실어도 안전한 문자열 하나로 직렬화하는 "형식(format)"**이며, 실제 서명은 JWS(§4)가, 암호화는 JWE(§5)가 담당합니다. 그런데 JWS든 JWE든 **"JSON을 Base64Url로 인코딩해 점(`.`)으로 잇는다"는 직렬화 원리는 동일**합니다. 그래서 이 공통 형식을 먼저 세우고, 뒤에서 서명(§4)·암호화(§5)의 차이만 다루겠습니다.

### 파트를 점(`.`)으로 이어붙인 단일 문자열

JWT은 여러 파트로 나뉘며, 각 파트는 내부적으로 JSON 형태를 띠지만 **주고받을 때 JSON의 괄호 `{ }` 모양 그대로 전달되지 않습니다.** 실제로는 각 파트를 Base64Url로 인코딩한 뒤 마침표(`.`)로 이어붙인 **하나의 문자열**로 최종 발급됩니다.

| 구현 | 파트 수 | 형태 |
|------|--------|------|
| **JWS**(서명) | 3부분 (점 2개) | `Base64Url(Header).Base64Url(Payload).Base64Url(Signature)` |
| **JWE**(암호화) | 5부분 (점 4개) | `Base64Url(Header).…(EncryptedKey).…(IV).…(Ciphertext).…(AuthTag)` (§5) |

아래는 실제 HTTP 헤더에 실린 JWS 형태 JWT입니다(전부 한 줄).

```http
GET /connect HTTP/1.1
Host: relay.example.com
Authorization: Bearer eyJhbGciOiJFUzI1NiIsImtpZCI6Imlzc3Vlci0yMDI2LTA3LWEifQ.eyJzdWIiOiJzdmM6OldlYlNlcnZlciIsImV4cCI6MTc1MTQxNDcwMH0.MEUCIQDx7k2...
                       └                    Header                  ─┘ └────                   Payload                  ────┘ └─  Sig   ─┘  (전부 한 줄)
```

### JSON → Base64Url → HTTP: 어떻게 문자열이 되나

```mermaid
flowchart TD
    A["<b>1단계 · 원본 JSON</b><br/>Header { alg, kid } · Payload { sub, exp } · Signature(원시 바이트)"]
    B["<b>2단계 · 각 파트를 Base64Url 문자열로</b><br/>eyJhbGci... · eyJzdWI... · MEUCIQDx7k2..."]
    C["<b>3단계 · 마침표(.)로 결합한 최종 문자열</b><br/>eyJhbGci....eyJzdWI....MEUCIQDx7k2...  → HTTP 헤더에 실림"]
    A -->|" 각 파트 Base64Url 인코딩 "| B
    B -->|" 마침표(.)로 결합 "| C
```

### Payload는 "숨겨지지" 않는다 — 인코딩 ≠ 암호화

여기서 가장 중요한 점: **Payload는 암호화되지 않습니다.** Base64Url은 암호화가 아니라 **인코딩**일 뿐이라, 누구나 즉시 디코딩해 그 안의 내용을 읽어볼 수 있습니다. `eyJ...`로 보여 암호화된 듯하지만 착각입니다. 그래서 (§4에서 볼) **JWS의 서명은 데이터를 숨기기 위함이 아니라 오직 내용의 "위변조 여부"를 증명**하는 역할을 합니다. 내용을 정말 숨겨야 한다면 Payload 자체를 암호화하는 JWE(§5)를 써야 합니다.

### Base64Url을 쓰는 이유 - URL·헤더에서 안전

JWT은 URL·HTTP 헤더·쿠키에 실려 다닙니다. 그런데 일반 Base64가 쓰는 `+` `/` `=` 세 문자는 이들 맥락에서 **특수 의미**를 가져 깨집니다. Base64Url은 이를 치환해 해결합니다.

| 문자   | 일반 Base64 | Base64Url | 치환 이유                      |
|------|-----------|-----------|----------------------------|
| 62번째 | `+`       | `-`       | URL에서 `+`는 공백(space)으로 해석됨 |
| 63번째 | `/`       | `_`       | `/`는 URL 경로 구분자로 오인됨       |
| 패딩   | `=`       | **제거**    | `=`는 쿼리스트링 `key=value`와 충돌 |

```text
// 같은 원본, 다른 인코딩
원본 바이트 → 일반 Base64 :  "a+b/c8=="
원본 바이트 → Base64Url   :  "a-b_c8"   (+→-, /→_, 패딩= 삭제)
```

> 💡 **왜 굳이 인코딩을 거치나? - HTTP 헤더·URL에 안전하게 싣기 위함이다.** JSON 원문에는 `{` `}` `"` 공백·줄바꿈 등 헤더·쿼리 파라미터에서 깨지거나 재해석되는 문자가 많습니다. 토큰은 `Authorization` 헤더나 쿼리스트링에 실려야 하므로, **어느 맥락에 넣어도 안전한 문자 집합(`A–Z a–z 0–9 - _`)으로 먼저 변환**해 두는 것입니다.

> ⚠️ **주의:** Base64Url은 **인코딩**일 뿐 **암호화가 아닙니다.** Payload가 `eyJ...`로 보여 암호화된 듯하지만 누구나 즉시 디코딩해 읽을 수 있습니다(위 "Payload는 숨겨지지 않는다" 참조).

> ↳ 단, JWK/JWKS(§3)는 헤더가 아니라 **응답 바디(JSON)**로 오가므로 인코딩이 필요 없어 평문 그대로 전송됩니다.

## [3. JWK: 키를 표현하는 순수 JSON 규격](https://datatracker.ietf.org/doc/html/rfc7517)

JWK는 공개키/개인키를 **인코딩 없이 평문 JSON**으로 표현합니다.

검증자(시그널링 서버, Resource Server)는 발급자(발급 서버, 인증 서버)의 **공개키 JWK**를 받아 서명을 검증합니다.

### [JWK Format: 주요 필드](https://datatracker.ietf.org/doc/html/rfc7517#section-4)

```json
{
  "kty": "EC",
  "kid": "issuer-2026-07-UUID",
  "crv": "P-256",
  "alg": "ES256",
  "use": "sig",
  "x": "f83OJ3D2xF1Bg8vub9tLe1gHMzV76e8Tus9uPHvRVEU",
  "y": "x_FEzRu9m36HLN_tue659LNpXW6pCyStikYjKIWI5a0"
}
```

_Example 공개키 JWK 예시 (검증자가 받는 것): EC P-256 기준_

| 필드       | 의미                                          | 예시 값                           |
|----------|---------------------------------------------|--------------------------------|
| `kty`    | Key Type, 키 유형                              | `EC` (타원곡선), `RSA`, `oct`(대칭키) |
| `kid`    | Key ID, 키 식별자. 키 롤링 시 어떤 키로 검증할지 지목         | `"issuer-2026-07-UUID"`        |
| `crv`    | Curve, 타원곡선 종류 (kty=EC일 때)                  | `"P-256"`                      |
| `alg`    | 알고리즘                                        | `"ES256"`                      |
| `use`    | 공개 키 용도<br/> - `sig`: 서명 <br/> - `enc`: 암호화 | `"sig"`                        |
| `x`, `y` | 공개키 좌표 (Base64Url 인코딩된 곡선 위의 점)             | `"f83OJ3D2..."`                |
| `d`      | **개인키** 값. 공개키 JWK에는 **절대 포함 금지**           | 발급 서버만 보유                      |

발급 서버는 `ECKeyGenerator(Curve.P_256)` 류의 API로 EC 키쌍을 생성하고 `kid`를 부여합니다.
다수의 공개키 JWK를 배열로 묶은 것이 **JWKS**이며, 키 롤링시 신, 구 키를 함께 노출해 무중단 교체를 가능케 합니다.


> `EC P-256` 란 타원곡선 암호화(ECC)의 표준
> - [RFC 5349: ECC(타원 곡선 암호화)](https://datatracker.ietf.org/doc/html/rfc5349)

```http
GET /.well-known/jwks.json HTTP/1.1

HTTP/1.1 200 OK
Content-Type: application/json

{
    "keys": [
        {
            "kty":"EC", "crv":"P-256",
            "kid":"issuer-2026-07-a",
            "x":"f83OJ...",
            "y":"x_FEz..."
        }
    ]
}
```

- JWK/JWKS는 인코딩 없이 "평문 JSON 그대로" 전송된다.
    - 응답: Base64Url 아님, JSON 평문 그대로
    - JWS, JWE는 전송 시 Base64Url 문자열 덩어리(`eyJ...`)로 변환되지만, **JWK와 JWKS는 예외**다.
- 검증자가 공개키를 가져올 때 **순수 JSON 본문 그대로** 응답으로 받는다. (공개키는 애초에 공개돼도 되는 정보라 숨길 이유가 없다.)

## 4. JWS - 서명 메커니즘: 내용은 투명, 위변조만 방지

§2의 JWT 형식을 **"서명"으로 구현**한 것이 JWS입니다(3부분 구조·Base64Url 직렬화는 §2 참조). Payload는 공개되어 누구나 읽을 수 있고(§2), 서명은 오직 **무결성·진위(위변조 방지)**만 보장합니다. 이 아키텍처가 쓰는 것이 바로 이 JWS입니다.

세 파트가 실제로 담는 내용은 다음과 같습니다.

- Header - 서명 알고리즘과 키 지목
    ```json
    // kid 로 "JWKS 중 어떤 공개키로 검증하라"를 지목. JWK 전체가 아니라 kid만 넣는다.
    {
      "alg": "ES256",
      "typ": "JWT",
      "kid": "issuer-2026-07-a"
    }
    ```
- Payload - 클레임(Claim) 집합
    ```json
    {
      "iss": "https://issuer.example.com",  // 발급자
      "sub": "svc::WebServer",              // 주체(역할)
      "authority": { "sub": ["topic/..."], "pub": ["topic/..."] },
      "iat": 1751414400,            // 발급 시각
      "exp": 1751414700             // 만료 시각
    }
    ```
- Signature - 무결성 증명
    ```text
    // 검증자는 공개키(x,y)로 이 서명을 검증. 한 글자라도 바뀌면 검증 실패.
    Signature = ECDSA_SHA256(
        Base64Url(Header) + "." + Base64Url(Payload),
        발급 서버의 개인키(d)
    )
    ```

> 검증자(시그널링 서버)는 §3에서 받은 **공개키 JWK**를 `kid`로 골라 서명을 검증합니다. Payload를 한 글자라도 바꾸면 서명이 깨져 즉시 탐지되지만, **내용을 읽는 것 자체는 막지 못합니다**(그것이 JWS의 설계 의도). 최종적으로 `eyJ....eyJ....MEUC...` 형태의 단일 문자열로 직렬화되는 과정은 §2를 참조하세요.

## 5. JWE - Payload 자체를 암호화 (기밀성)

JWE는 내용을 **암호화**해 기밀성을 보장합니다. 이는 전송 계층 암호화(TLS)와는 별개인 **애플리케이션 계층 암호화**로, 중간 저장소나 로그에 남아도 내용이 노출되지 않습니다. 구조는 마침표로 구분된 **5단**입니다.

```mermaid
flowchart LR
    H["①Header"] --- K["②EncryptedKey"] --- I["③IV"] --- CT["④Ciphertext"] --- T["⑤AuthTag"]
```

| 단 | 구성요소               | 설명                                        |
|---|--------------------|-------------------------------------------|
| ① | JOSE Header        | `alg`(키 암호화 방식) · `enc`(본문 암호화 방식)        |
| ② | Encrypted Key      | 본문 암호화에 쓸 **대칭키(CEK)**를 수신자 **공개키**로 감싼 것 |
| ③ | IV                 | 초기화 벡터 (동일 평문의 반복 암호화 방지)                 |
| ④ | Ciphertext         | 대칭키로 암호화된 실제 Payload                      |
| ⑤ | Authentication Tag | 암호문 변조 탐지용 무결성 태그 (AEAD)                  |

### 대칭키 · 비대칭키 혼합(Hybrid) 원리

> 💡 **왜 섞는가?** 비대칭 암호화는 느리고 큰 데이터에 부적합, 대칭 암호화는 빠르지만 키 공유가 어렵습니다. JWE는 둘의 장점만 취합니다.
> 1. 본문은 임의 생성한 **대칭키(CEK)**로 빠르게 암호화 (단 ④)
> 2. 그 대칭키를 수신자 **공개키(비대칭)**로 안전하게 감싸 전달 (단 ②)
> 3. 수신자는 자신의 **개인키**로 대칭키를 풀고 → 대칭키로 본문 복호화

```json
// JWE Header(①) 원본 JSON - RSA로 대칭키를 감싸고, 본문은 AES-GCM으로 암호화
{
  "alg": "RSA-OAEP-256",
  "enc": "A256GCM",
  "kid": "verifier-enc-01"
}
```

**JWS와 다른 점:** JWS는 점 **2개(3부분)**, JWE는 점 **4개(5부분)**. 하지만 §2의 직렬화 원리(JSON → Base64Url → 점 결합)는 동일합니다. 차이는 JWE의 ④Ciphertext가 암호화돼 있어 **Base64Url을 풀어도 원본을 읽을 수 없다**(JWS Payload는 풀면 바로 읽힘)는 것뿐입니다.

이 아키텍처의 현재 구조는 JWS(서명)만 사용하며 JWE는 쓰지 않습니다. 권한 정보는 민감정보가 아니고 TLS로 전송 구간이 보호되기 때문입니다. JWE는 향후 Payload에 기밀 데이터를 실어야 할 경우의 선택지로 남겨둡니다.

---

# Part II · 현재 아키텍처

> 🔴 **역할 정의:** 이 문서에서 인증 관련 세 서버 역할은 다음과 같이 부릅니다.
> · **발급 서버(Issuer)** = 개인키 보유 = 토큰 서명·발급 (OAuth의 Authorization Server. 통상 백엔드/웹앱 서버가 겸함)
> · **시그널링 서버(Verifier)** = 공개키 보유 = 서명 검증 = 세션 제어 (실시간 메시징 브로커, 예: MQTT over `wss://`)
> · **데이터 중계 서버** = 실제 스트림 데이터(화면·제어) 전달 (실시간 저지연 채널, 예: TCP)

### 서버 역할 분리 - 시그널링 vs 데이터 중계 (매우 중요)

흔히 "중계 서버"로 통칭하지만, 실시간 원격 제어(또는 스트리밍) 경로는 **역할이 다른 두 서버**로 분리되는 것이 일반적입니다. 이 구분을 놓치면 문제의 원인(§8)과 해결 대상(§14)을 엉뚱한 서버에 조준하게 됩니다.

| 구분         | ① 시그널링 서버 (메시징 브로커)         | ② 데이터 중계 서버                       |
|------------|---------------------------------|-------------------------------------|
| 역할         | **세션 제어 + 토큰 인증**             | **실제 화면·제어 데이터 전달**               |
| 무엇이 흐르나    | 세션 수립·제어 신호, JWS 인증             | 클라이언트 ↔ 대상(에이전트) 간 실시간 제어 스트림     |
| 전송 보안 (전형) | 🟢 `wss://` 적용 - 암호화되어 안전       | 🔴 `tcp://` 평문 구간이 남기 쉬움 (§8 원인)   |
| 인증         | 여기서 발급 토큰을 검증                   | 연결 시 세션 ID(또는 토큰)로 세션 식별           |

> 💡 **핵심 분업:** **인증(JWS)은 시그널링 서버에서**, **실데이터는 데이터 중계 서버에서** 처리된다. 따라서 "인증 강화"와 "전송 암호화"는 **서로 다른 서버에 적용해야 하는 별개 과제**다 - 이 둘을 한 서버 문제로 뭉뚱그리면 안 된다.

## 6. 현재 구조와 그 이유

### 핵심 사실: 시그널링 서버(검증자)에는 DB가 없다

발급 서버가 발급 주체가 된 것은 우연이 아니라 다음 제약에서 온 **실무적 타협**입니다.

1. **검증자에 저장소가 없음** - 사용자·에이전트 관계를 조회할 DB가 없어 상태 없는(stateless) 검증만 가능. 권한 정보를 토큰 Payload에 실으면 DB 없이 검증됨.
2. **상용 메시징 브로커** - 검증자가 상용 브로커라면 JWT 발급 로직을 커스텀 삽입하기 어렵고 운영 복잡도가 급증함.
3. **패치 주기 분리** - 웹(발급 서버) 패치는 자유롭지만 브로커는 인프라라 자주 못 바꿈. 인증 로직을 발급 서버에 두면 발급 서버만 패치하면 됨.

### 왜 클라이언트가 토큰을 직접 들고 다니는가 - 무상태·초고속 검증

이 구조의 진짜 설계 의도는 "편의"가 아니라 **대규모 트래픽에서의 검증 성능**입니다. 실시간 세션은 수많은 클라이언트가 동시에 검증자에 붙습니다. 이때 검증을 위해 아래 두 경로를 택했다면 모두 병목·의존을 만듭니다.

| 택하지 않은 방식                                                              | 왜 안 쓰는가 (단호히)                                                                                                       |
|-----------------------------------------------------------------------|---------------------------------------------------------------------------------------------------------------------|
| 검증자 ↔ 발급 서버 **이종 서버 간 검증(S2S)**<br>(검증자가 접속마다 발급 서버에 "이 토큰 유효?" 질의)   | 접속 1건마다 서버 간 왕복(RTT)이 붙어 **지연이 트래픽에 비례해 폭증**. 발급 서버가 검증 트래픽의 단일 병목·단일 장애점이 된다. 대규모 동시 접속에서 사실상 불가.                  |
| 클라이언트가 **검증 API를 발급 서버에 직접 호출**<br>(붙기 전에 물어보고 붙기)                     | 검증 주체를 발급 서버로 되돌리는 것이라 **JWS를 쓰는 의미 자체가 사라진다.** 매번 왕복 + 의존 → stateless 이점 소멸. "빠르게, 서버 간 통신 없이" 검증하려는 목적과 정면 충돌.  |

> 💡 **그래서 선택한 것:** 클라이언트가 **위조 불가능한 "출입증"(JWS)**을 직접 들고 다니고, 검증자는 **손에 든 공개키만으로 그 자리에서(로컬) 검증**한다.
> - **무상태(Stateless) 검증** - 검증자는 DB도, 발급 서버 질의도 필요 없이 토큰 서명만 검증한다. (단, §11 DPoP 도입 시 `jti` 재사용 방지 목적의 **단기 상태**만 예외로 검증 서버가 가진다 - "검증 로직은 무상태 + `jti` 방지만 단기 상태".)
> - **초고속·수평 확장** - 검증이 CPU 로컬 연산이라 지연이 트래픽과 무관. 검증자 인스턴스를 늘려도 공유 저장소가 필요 없다.
> - **서버 간 통신 0회** - 공개키(JWKS)는 시작 시/주기적으로 한 번만 받아 캐싱하면, 이후 접속마다의 발급 서버 통신이 없다.

즉 "발급 서버 발급 → 클라이언트 소지 → 검증자 로컬 검증"은 **이종 서버 간 실시간 검증을 없애기 위한** 의도적 설계입니다. 검증을 발급 서버로 되돌리는 어떤 방식(S2S·클라이언트의 검증 호출)도 이 이점을 무너뜨립니다.

### 구현 예시 (의사코드)

발급 서버가 EC P-256 키쌍을 생성·보관하고, 역할별로 JWT를 발급하며, 실시간 연결에 자격 증명을 주입하는 흐름은 언어·프레임워크 무관하게 다음과 같습니다.

```text
// ① 발급 서버: EC P-256 키쌍 생성 (kid, 발급시각 부여)
keyPair = generateKeyPair("EC", "P-256"); keyPair.kid = newKid()

// ② 역할별 JWT 발급 (권한을 클레임에 실음)
authority = { sub: subscribeTopics, pub: publishTopics }
token = sign({ id: "svc::WebServer", authority, exp }, keyPair.private)

// ③ 실시간 연결(메시징/WebSocket)에 자격 증명 주입
connectOptions.username = connId
connectOptions.password = token
```

## 7. TLS 인증서 vs JWK - 다른 계층, 다른 목적

| 구분        | TLS 인증서       | JWK (EC P-256)     |
|-----------|---------------|--------------------|
| 계층        | 전송 계층 (연결 보호) | 애플리케이션 계층 (메시지 진위) |
| 목적        | "도로(연결)"를 암호화 | "택배(JWT)"의 진위를 증명  |
| 주인        | 검증자(시그널링) 서버  | 발급 서버 (직접 생성)      |
| 대표 알고리즘   | RSA (X.509)   | ECDSA (P-256 곡선)   |
| 발급 서버 보유  | 검증자 공개 인증서만   | 개인키 + 공개키 전체       |

둘은 대체 관계가 아니라 **상호 보완**입니다. TLS가 뚫려 트래픽이 노출돼도 JWS 서명 덕분에 위조 토큰은 검증에서 걸립니다.

## 8. 문제점 분석

| 문제                             | 설명                                                                                              | 위험도   |
|--------------------------------|-------------------------------------------------------------------------------------------------|-------|
| **인증/인가 부재 (Broken Access Control)** | **세션 ID(room id)만으로 세션 접속을 허용** → 소유자·권한 검증이 없어 남의 세션 ID를 획득한 누구나 그 세션에 접속·열람 가능. **이것이 근본 원인**(아래 상술) | 🔴 치명(근본) |
| 데이터 중계 구간 평문(`tcp://`)          | 데이터 중계 서버에 붙는 구간이 평문 TCP → 세션 ID·토큰이 스니핑됨. **근본 원인은 아니고, 세션 ID를 얻는 여러 경로 중 하나**(위험 지점)          | 🔴 치명 |
| 키 롤링 미구현                       | 단일 키만 사용. 교체 시 발급된 모든 토큰이 일시 무효화                                                                | 🟠 높음 |
| 만료(exp) 없음                     | 서버/에이전트/클라이언트 토큰에 exp 미설정 → 탈취 시 무기한 유효                                                        | 🟡 중간 |
| 발급 서버 단일 장애점                   | 발급 서버 다운 시 신규 JWT 발급 불가 (기존 연결은 유지)                                                            | 🟡 중간 |
| 표준 클레임 미검증                     | `aud`(토큰 사용 대상)·`jti`(재사용 방지 식별자) 미검증 - OAuth2 리소스 서버 표준 이탈                                     | 🟡 중간 |

시그널링 구간은 **이미 `wss://`로 보호**되고 있다고 가정합니다. 그러나 아래에서 보듯 **진짜 위험은 전송 암호화 여부가 아니라, 세션 ID만 있으면 소유자 확인 없이 세션에 붙을 수 있다는 인가 설계 자체**에 있습니다. "만료 없음"·"바인딩 없음"은 **리플레이 공격** 표면으로 Part III에서 다룹니다.

### 실제 사고 원인 규명 - 근본 원인은 스니핑이 아니라 인증/인가 부재

> 🔴 **가장 중요한 정정:** 이 사고를 "TCP 평문 때문에 세션 ID가 스니핑됐다"로만 진단하면 **절반만 맞고, 근본 원인을 놓친다.** 진짜 원인은 **세션 ID(room id)만 있으면 소유자·권한 확인 없이 남의 세션에 붙을 수 있다는 인가 설계(Broken Access Control)**다. 스니핑은 그 세션 ID를 얻는 **여러 경로 중 하나**일 뿐이다.

**실제 공격 체인** — 스니핑 없이도 성립했다.

1. **TURN 서버 응답에서 room id 획득** — 세션 협상 과정에서 받은 응답 데이터에 room id가 담겨 있어, 이를 통해 다른 사람이 원격 제어 중인 room id를 얻을 수 있었다.
2. **MQTT(PNS) 구독 감청** — 구독 중인 MQTT 서버에서 **다른 사람이 구독 중인 토픽 데이터를 그대로 감청**할 수 있었다(토픽 구독에 대한 권한 검증 부재).
3. **획득한 room id로 중계 서버 접속 → 남의 세션 열람** — 그 room id로 중계 서버에 붙으면, **소유자 확인 없이** 다른 사람이 원격 제어 중인 화면을 그대로 볼 수 있었다.

```mermaid
flowchart TD
    A["공격자"] -->|① TURN 응답에서 room id 획득| R["room id (남의 세션 식별자)"]
    A -->|② MQTT 구독 감청<br/>(토픽 권한 검증 없음)| R
    R -->|③ 이 room id로 중계 서버 접속| Relay["데이터 중계 서버"]
    Relay -->|"소유자 확인 없음 → 접속 허용"| V["🔴 남의 원격 세션 열람"]
    style V fill:#f8d7da,stroke:#dc3545
    style Relay fill:#fff3cd,stroke:#ffc107
```

핵심은 **room id가 사실상 "소지하면 곧 접근 권한"인 Bearer capability로 동작**했다는 점입니다. room id는 애초에 협상·구독 과정에서 여러 주체에게 노출되는 식별자인데, 중계 서버가 "이 room id의 **정당한 소유자인가**"를 검증하지 않으니, room id를 어떤 경로로든 얻은 사람은 모두 남의 세션에 붙을 수 있었습니다.

> ⚠️ **그래서 TLS(전송 암호화)만으로는 근본 해결이 안 된다.** TLS는 ①번 경로(평문 스니핑)를 막지만, room id는 **TURN 응답·MQTT 감청 등 암호화된 경로에서도 새어 나갑니다.** 전송을 암호화해도 "room id만 있으면 붙을 수 있다"는 인가 구멍은 그대로입니다. **필요조건이지 충분조건이 아닙니다.**

**계층별로 막는 것이 다릅니다.** 스니핑은 여전히 실재하는 위험 지점이므로 아래 표는 유효합니다. 다만 이것이 문제의 전부가 아님을 유의하세요.

| 계층          | 수단                  | 막는 것                                        | 못 막는 것                                  |
|-------------|---------------------|---------------------------------------------|-----------------------------------------|
| 전송 계층 (TCP) | **TLS / SSL / WSS** | 도청(스니핑) - 평문 구간에서의 세션 ID·토큰 노출 차단           | **다른 경로(TURN/MQTT)로 얻은 식별자의 재사용**       |
| 애플리케이션 계층   | JWS (서명)            | 위·변조                                        | **소지자 신원 확인 - "누가 쥐었는지"는 안 봄(Bearer)**  |
| 애플리케이션 계층   | **PoP / DPoP (소유 증명)** | **획득한 식별자·토큰의 무단 사용 - 개인키 없으면 접속 불가**       | (전송 도청 자체는 TLS 몫)                        |

> 🔴 **근본 해결책은 인가 모델을 Bearer → 소유 증명(PoP)으로 바꾸는 것입니다.** 세션을 **클라이언트의 키 소유에 결착**하면, 공격자가 room id를 어떤 경로로 얻더라도 **개인키가 없어 접속이 거부**됩니다(Part III 방안 B). 여기에 **MQTT 토픽 구독 권한 검증**(발급 토큰의 `authority` 클레임으로 자기 토픽만 구독 허용)을 더하면 감청 경로(②)도 닫힙니다.

### 왜 인프라(TLS)와 애플리케이션(PoP·인가)을 반드시 "병행"해야 하는가 - 직교하는 두 속성

**TLS와 PoP는 대체 관계가 아니라 직교(orthogonal) 관계입니다.** 서로 다른 보안 속성을 지키므로 하나가 다른 하나를 포섭하지 못하고, **어느 하나만 적용하면 각기 다른 실재 구멍이 남습니다.**

| 적용한 것 | 지켜지는 속성 | **남는 구멍 (실재 위협)** |
|-----------|--------------|--------------------------|
| **TLS만** (전송 암호화) | 전송 기밀성·무결성 | 🔴 **인가 구멍** - room id를 TURN 응답·MQTT 감청 등 **암호화된 정상 경로로 얻어** 재사용 → 소유자 확인 없이 남의 세션 접속 |
| **앱(PoP·인가)만** (TLS 없음) | 접속 주체의 소유권·권한 | 🔴 **기밀성·무결성 구멍** - ① 접속은 막아도 **평문 스트림을 수동 도청**해 원격 화면 그대로 열람 ② **DPoP는 스트림 본문을 서명하지 않으므로** 핸드셰이크 통과 후 평문 구간을 능동 MITM이 **변조·주입** 가능 |
| **둘 다 (병행)** | 소유권 + 전송 기밀성·무결성 | ✅ **근본 해결** - 획득한 식별자는 개인키 없이 무용, 스트림은 도청·변조로부터 보호 |

- **TLS = "무엇이 흐르는가"를 보호**(스트림 기밀성·무결성). PoP로도 이건 못 막는다 — DPoP는 **연결 핸드셰이크만 인증하지 스트림 본문은 서명하지 않기** 때문.
- **PoP·인가 = "누가 붙는가"를 보호**(접속 소유권·권한). TLS로도 이건 못 막는다 — room id는 도청이 아니라 **정상 응답·구독 데이터**로 새기 때문.

> ⚠️ **오해 주의(양방향):** "PoP만 하면 접속을 막으니 TLS는 나중에"는 **틀린 추론**입니다 — 접속을 막아도 평문 스트림 도청으로 **원격 화면이 그대로 노출**되고 스트림 무결성 보장도 없습니다. 반대로 "TLS만 하면 안전"도 틀립니다(표 1행) — room id가 정상 경로로 새면 인가 구멍이 그대로입니다. **둘은 각자 다른 구멍을 막으므로 생략 불가·병행 필수입니다.**

**우선순위 재정의:** TLS(P0)는 스니핑을 없애고 **스트림 기밀성·무결성을 보장하는 필요조건**이고, 소유 증명(PoP) + 토픽 권한 검증은 **근본 원인(인가 부재)을 없애는 핵심 처방**입니다. 어느 하나도 "나중에 검토할 옵션"이 아니라 **둘 다 필수이며 병행**해야 합니다(§14).

---

# Part III · 토큰 탈취 방어 개선안

## 9. 리플레이 공격 모델

공격자가 정상 토큰(JWS)을 어떤 경로로든(로그, 프록시, 감염 단말) 탈취하면, 서명이 유효하므로 **그대로 재전송(replay)**해 정당한 사용자로 위장할 수 있습니다. 순수 JWS는 "누가 소지했는가"를 검증하지 않고 "서명이 유효한가"만 보기 때문입니다(Bearer 토큰의 한계).

> 💡 **전제 (§8에서 이어짐):** 아래 방어는 모두 **전송 구간 TLS가 이미 적용되었다**는 위에서만 유효하다. TLS로 "도청에 의한 탈취"를 먼저 차단한 뒤, 그럼에도 남는 "종단 탈취 후 재사용(리플레이)"을 아래 바인딩/DPoP로 무력화하는 순서다.

> 🔴 **핵심 방어 원리:** 토큰을 **특정 소지 증거에 묶어(bind)**, 탈취되어도 다른 주체가 못 쓰게 만든다. 아래 두 방안은 "무엇에 묶는가"가 다릅니다 - **방안 A는 단말의 속성**에, **방안 B는 클라이언트의 개인키 소유**에 묶습니다.

## 10. 방안 A - 단말 고유 정보 바인딩

JWT Payload 클레임에 클라이언트 고유 정보(IP · MAC · Device ID)를 박아두고, 검증자가 접속 요청의 실제 정보와 **교차 검증**합니다.

### ① 발급 시 - 클레임에 단말 정보 삽입

```json
// JWS Payload
{
  "sub": "agent-8842",
  "exp": 1751414700,
  "bnd": {                        // binding 클레임(커스텀)
    "ip": "203.0.113.45",
    "did": "DEV-9f2c1a7b"         // Device ID
  }
}
```

### ② 검증 시 - 접속 정보와 교차 대조

```text
// 요청의 소스 IP / 기기 헤더 vs 토큰 클레임
if (request.sourceIp != token.bnd.ip)               reject("IP mismatch");
if (request.header("X-Device-Id") != token.bnd.did) reject("device mismatch");
```

### HTTP 요청 예

```http
GET /relay/connect HTTP/1.1
Host: relay.example.com
X-Device-Id: DEV-9f2c1a7b
Authorization: Bearer eyJ...(bnd 클레임 포함)...
```

| 항목     | 평가                                                         |
|--------|------------------------------------------------------------|
| 구현 난이도 | 🟢 낮음 - 클레임 추가 + 비교 로직뿐, 검증자에 저장소 불필요                      |
| 강점     | 기존 JWS 구조 유지, stateless 검증 유지                              |
| 약점     | IP는 NAT·모바일·프록시 환경에서 **정상 변동** → 오탐 위험. MAC은 스푸핑 가능·수집 어려움 |
| 적합 상황  | 고정 IP 에이전트, 내부망 등 네트워크 환경이 안정적인 경우                         |

> ⚠️ **한계:** 공격자가 단말 정보까지 함께 탈취·위조하면 무력화됩니다. 클레임 값은 결국 "공격자가 헤더에 넣을 수 있는 값"이기 때문입니다. 암호학적 소유 증명이 아닙니다 → 그 한계를 메우는 것이 방안 B.

## 11. 방안 B - DPoP (소유 증명 · Proof-of-Possession)

### 먼저 개념부터 - 두 눈높이로

> 🟢 **신입 개발자용 (Easy) - "지문 인식 호텔 카드키"**
> - **일반 JWS는 그냥 호텔 카드키**다. 발급 서버가 발급해 주고, 문(검증자)은 "카드가 진짜인가"만 본다. **주우면 아무나 문을 연다.** 이게 지금 구조의 약점(리플레이).
> - **DPoP는 "지문 인식 카드키"**다. 카드(토큰)에 **내 지문(공개키 지문)**이 새겨져 있고, 문을 열 때마다 **실제 내 손가락(개인키)**을 대야 열린다.
> - 그래서 **카드를 훔쳐도 소용없다.** 도둑에겐 내 손가락(개인키)이 없으니까. 개인키는 내 기기 밖으로 **절대 나가지 않는다.** 훔칠 카드에 애초에 손가락이 안 담겨 있다.

> 🔵 **시니어 개발자용 (Deep) - Cryptographic Key-Binding**
> - **Ephemeral Key Pair**: 클라이언트가 세션 단위로 EC P-256 **임시 키쌍**을 생성한다. 개인키는 프로세스 메모리(또는 보안 저장소)에만 존재하고 네트워크로 반출되지 않는다.
> - **JWK Thumbprint (RFC 7638)**: 공개키 JWK의 정규(canonical) 필드를 정렬·직렬화해 SHA-256 해시한 값(`jkt`). 공개키의 고정 지문이며, 발급 서버는 이 `jkt`를 Access Token의 `cnf`(confirmation) 클레임에 박제한다.
> - **Cryptographic Key-Binding**: 이로써 토큰은 특정 키쌍에 **암호학적으로 결착**된다. Bearer(소지자) 모델에서 **Sender-Constrained(발신자 구속)** 모델로 전환되어, 토큰 탈취만으로는 사용이 불가능해진다.
> - 검증자는 매 요청의 **Proof JWT 서명 검증 + `jkt` 크로스 매칭**을 순수 CPU 연산으로 수행 → 무상태 검증 정신(§6) 유지, `jti` 재사용 방지용 단기 캐시만 로컬에 둔다.

**역할 매핑:** 발급은 **발급 서버(Issuer)**가, 소유 증명 검증은 **세션에 접속하는 지점의 검증자**가 담당합니다. 이 사고의 근본 원인이 "**데이터 중계 서버에 room id만으로 접속 가능**"이었으므로, **소유 증명 검증은 반드시 데이터 중계 접속 지점(순수 TCP)에 있어야** 합니다. 데이터 중계는 HTTP 헤더가 없으므로 Proof는 **연결 직후 핸드셰이크 프레임(바이트 스트림)**으로 전달합니다. (시그널링이 WebSocket이면 최초 upgrade 핸드셰이크 헤더도 병행 가능)

### 세션 수립 6단계 시퀀스

```mermaid
sequenceDiagram
    participant C as Client (뷰어/호스트)
    participant W as 발급 서버 (Issuer)
    participant R as 검증자 (데이터 중계 접속 지점)
    Note over C: ① 임시 키쌍 생성<br/>(priv 은닉, pub=JWK)
    C ->> W: ② 원격제어 요청 + 공개키(JWK) 전달
    Note over W: ③ jkt = SHA256(JWK) 연산<br/>cnf.jkt 박제해 토큰 발급
    W -->> C: Access Token (cnf 포함)
    Note over C: ④ Proof JWT 생성<br/>(htm/htu 매핑·iat·jti, priv로 서명)
    C ->> R: ⑤ 핸드셰이크 프레임 전송<br/>토큰 + Proof (HTTP 헤더 아님)
    Note over R: ⑥ 3중 교차 검증
    R -->> C: 통과 시 세션 수립
```

#### 1단계 · 클라이언트 임시 키쌍 생성 (개인키 은닉)

클라이언트(뷰어·호스트)가 세션 시작 시 EC P-256 키쌍을 만든다. **개인키(priv)는 기기 메모리 밖으로 절대 나가지 않는다.** 공개키(pub)만 JWK 형태로 다음 단계에 실린다.

```text
// Client - 런타임 키쌍 생성 (pseudo)
keyPair    = generateKeyPair("EC", "P-256")
privateKey = keyPair.private   // 메모리에만 보관, 반출 금지
publicJwk  = keyPair.public.toJWK()   // { kty:"EC", crv:"P-256", x, y }
```

#### 2단계 · 발급 서버에 원격제어 요청 + 공개키(JWK) 전달

```http
POST /api/remote/session HTTP/1.1     // 발급 서버 (wss/https)
Authorization: Bearer <사용자 로그인 세션/토큰>
Content-Type: application/json

{ "sessionId": "session-1234", "dpop_jwk": { "kty":"EC", "crv":"P-256", "x":"f83OJ...", "y":"x_FEz..." } }
```

#### 3단계 · 발급 서버의 지문 연산(SHA-256) + `cnf.jkt` 박제 발급

발급 서버는 받은 공개키 JWK로 **RFC 7638 Thumbprint(`jkt`)**를 계산해 Access Token의 `cnf` 클레임에 넣고 서명·발급한다.

```json
// Access Token(JWS) Payload - 발급 서버가 발급
{
  "sub": "agent-8842",
  "exp": 1751414700,             // 짧은 만료(§14 P1)
  "cnf": {
    "jkt": "0ZcOCORZNYy-DWpqq30jZyJGHTN0d2HglBV3uiguA4I"  // = SHA-256 thumbprint(공개키 JWK)
  }
}
```

#### 4단계 · 클라이언트 Proof JWT 생성 + 개인키 서명

클라이언트는 **연결(또는 요청)마다 즉석에서** Proof JWT를 만들어 **1단계의 개인키로 서명**한다. 무엇을(`htm`·`htu`), 언제(`iat`), 고유 id(`jti`)로 보내는지가 서명 대상이라 **가로채 재사용할 수 없다.**

> ⚠️ **순수 TCP에서의 `htm`/`htu` 매핑:** `htm`(HTTP 메서드)·`htu`(HTTP URI)는 **HTTP 전용 클레임**이다. 데이터 중계는 순수 TCP라 HTTP 메서드·URI가 없으므로, **채널 규격에 맞는 값으로 매핑을 정의**해야 한다 - 예: `htm`은 고정 동사(`"CONNECT"`), `htu`는 **대상 채널·세션 식별자**(`relay.example.com:9443/session-1234`). 검증자와 클라이언트가 이 매핑을 동일하게 약속해야 한다.

```text
// DPoP Proof JWT (연결마다 새로 생성) - 순수 TCP 매핑 예
Header  = { "typ":"dpop+jwt", "alg":"ES256", "jwk":{ 공개키 그대로 } }
Payload = { "htm":"CONNECT", "htu":"relay.example.com:9443/session-1234", "iat":1751414400, "jti":"c1d2-unique" }
Signature = ECDSA(Base64Url(Header)+"."+Base64Url(Payload), privateKey)  // ← 개인키 서명
```

#### 5단계 · 데이터 중계 서버로 핸드셰이크 프레임 전송 (헤더 아님)

데이터 중계는 순수 TCP라 `Authorization`/`DPoP` **HTTP 헤더를 쓸 수 없다.** 연결 직후 **릴레이 프로토콜이 정한 핸드셰이크 프레임**(길이 접두 바이너리 또는 JSON 등)에 토큰과 Proof를 담아 보낸다.

```text
// 순수 TCP 데이터 중계: 연결 직후 최초 1회 보내는 HELLO(핸드셰이크) 프레임
// 프레이밍은 릴레이 프로토콜 규격을 따른다(예: [4바이트 길이][JSON] 반복)

HELLO {
  "ver": 1,
  "access_token": "eyJ...(발급 서버 발급 Access Token, cnf 포함)...",   // ← "무엇을 할 수 있나"(권한)
  "dpop_proof":   "eyJ0eXAiOiJkcG9wK2p3dC...(Proof JWT, 개인키 서명)..."  // ← "내가 그 키의 주인"(소유)
}
// 검증 전에는 어떤 스트림 데이터도 흘려보내지 않는다(핸드셰이크 게이트).
```

> 시그널링이 WebSocket(`wss://`)인 경우엔 최초 upgrade가 HTTP라서 `Authorization: DPoP` / `DPoP:` 헤더를 그 핸드셰이크에 실을 수 있다. 그러나 **이 사고의 취약 지점은 데이터 중계(순수 TCP)**이므로, 위 프레임 검증이 핵심이다.

**두 헤더의 역할 분담:** `Authorization`은 발급 서버가 보증한 **권한**(누구·무엇), `DPoP`은 클라이언트가 매번 증명하는 **소유**(진짜 주인). 둘이 짝이 맞아야 통과한다.

#### 6단계 · 검증자의 3중 교차 검증

```text
// 검증자 - 순수 CPU 인메모리 연산 (외부 질의 없음)
1) 발급자 보증 검증: verify(AccessToken.sig, 발급 서버 공개키(JWKS))     // 토큰이 진짜 발급 서버 발급인가
2) 소유 권한 검증:  verify(DPoP.sig, DPoP.header.jwk)                   // 요청자가 그 개인키를 지금 쥐고 있나
3) 크로스 매칭:    SHA256(DPoP.header.jwk) == AccessToken.cnf.jkt ?     // 토큰의 주인 == 지금 서명한 자 인가
   + 부가 검사: htm/htu == 실제 요청 · iat 시간창 내 · jti 미재사용(단기 캐시)
```

> ✅ **왜 3중인가:** ①은 "위조 안 된 정상 토큰인가", ②는 "요청자가 키 주인이 맞나", ③은 "①의 토큰과 ②의 주인이 **동일인인가**"를 잇는다. 셋이 모두 통과해야만 "정당한 소유자의 정당한 토큰"이 성립한다. **탈취범은 ②(개인키)가 없어 반드시 여기서 걸린다.**

### "헤더를 통째로 복제하면?" - 복제 리플레이 방어와 그 책임 소재

> ⚠️ **정당한 의문:** 5단계의 두 헤더(`Authorization`+`DPoP`)를 **그대로 복사해 재전송**하면, 그 순간엔 정상 요청과 글자까지 동일하다. 그럼 리플레이와 뭐가 다른가?

다른 점은 **공격자는 "복제"만 되고 "새 Proof 생성"은 안 된다**는 것이다(개인키가 없으므로). 그래서 남는 공격은 "기존 Proof를 그대로 재전송"뿐이고, 이는 Proof 안의 세 값으로 막는다.

| 복제 시나리오               | 막는 장치                                      | 누가 검사    |
|-----------------------|--------------------------------------------|----------|
| 같은 Proof 즉시 재전송       | `jti`(1회용) + 검증 서버 인메모리 캐시 → "이미 쓴 id"로 거절 | **검증자**  |
| 캐시 만료 노려 지연 재전송       | `iat` 시간창(예 ±60초) 밖 → 거절                   | **검증자**  |
| 복제 Proof를 다른 URL에 재사용 | `htm`·`htu` 바인딩 → 그 메서드+URL 전용             | **검증자**  |

> 🔴 **책임 소재 - 복제 리플레이 방어는 발급 서버가 아니라 검증자 전담이다.**
> 발급 서버(Issuer)는 토큰을 발급하는 순간 손을 뗀다. 매 접속마다 오가는 Proof를 보는 것도, "이 `jti`를 방금 봤나"를 판단할 수 있는 것도 **검증자뿐**이다. 따라서 `jti` 재사용 방지는 구조적으로 검증자에만 놓일 수 있다.

| 주체         | `jti`에 대한 책임                                 | 상태           |
|------------|----------------------------------------------|--------------|
| Client     | 매 요청 **새 `jti` 생성**(난수)                      | 없음           |
| 발급 서버      | **관여 안 함** - `cnf.jkt` 박제까지가 끝               | 없음           |
| 검증자        | `jti` **기록 → 재전송 거절 → TTL 만료**(인메모리 생명주기 관리) | **단기 상태 보유** |

#### 검증자의 `jti` 라이프사이클 - TTL = `iat` 시간창

- 캐시 TTL을 `iat` 허용 시간창과 **일치**시킨다. 시간창보다 짧으면 만료 후 복제가 뚫리고, 길면 이미 `iat`로 거절될 것을 붙들어 메모리만 낭비한다.
- **다중 인스턴스 함정:** 로컬 메모리 캐시는 인스턴스 간 공유가 안 돼, 복제 Proof를 다른 검증자 인스턴스로 보내면 통과한다 → **짧은 TTL 공유 캐시(Redis 등)** 또는 **sticky routing**이 필요.

> 💡 **그래서 §6 "무상태"의 정확한 의미:** 검증자의 검증은 **대부분 순수 CPU 무상태**(서명 검증·`jkt` 매칭 - 외부 질의·저장 없음)지만, **`jti` 재사용 방지만은 단기 상태**를 가진다. 즉 "완전 무상태"가 아니라 **"검증 로직은 무상태 + `jti` 방지만 단기 상태"**가 정확한 서술이다.

> ✅ **더 강한 옵션 - 서버 Nonce(challenge-response, RFC 9449):** 검증자가 `DPoP-Nonce` 헤더로 **매번 새 난수(nonce)**를 내려주고, 클라이언트가 그 `nonce`를 Proof에 포함해 서명하게 하면, **사전 복제·시간창 내 재사용조차 원천 봉쇄**된다. `jti` 캐시/시간창 의존을 줄이는 상위 방어로, 리스크가 높은 경로에 선택 적용한다. (이 서버 nonce가 이 생태계에서 유일한 진짜 "서버 챌린지"이며, 클라이언트가 만드는 OIDC nonce·PKCE code_challenge와 생성 주체가 다르다.)

### 역할별 담당 R&R

각 파트가 당장 착수할 작업 스펙이다. 언어 무관 pseudo-code로 로직 흐름만 제시한다.

#### 🟧 발급 서버 담당

- **클라이언트 JWK 수신 API 설계** - 세션 개설 요청 바디에 `dpop_jwk`(공개키) 필드 수용. 필수값·형식(kty/crv/x/y) 검증, 잘못된 곡선·필드 거절.
- **RFC 7638 Thumbprint 연산** - 공개키 JWK의 **필수 필드만**(`{crv,kty,x,y}`) **사전순 정렬 + 공백 없는 JSON**으로 직렬화 후 SHA-256 → Base64Url. (라이브러리 있으면 재구현 금지: 예를 들어 Nimbus JOSE의 `ECKey.computeThumbprint()` 등.)
- **JWT `cnf` 클레임 주입** - 발급 시 `claims.cnf = { jkt: <thumbprint> }`. 기존 발급 로직에 `exp`(짧게)와 함께 추가.

```text
// 발급 서버 pseudo
jwk = request.body.dpop_jwk
validateEcP256(jwk)                        // 형식·곡선 검증
jkt = base64url(sha256(canonicalJson(jwk, ["crv","kty","x","y"])))
token = sign({ sub, exp: now+300, cnf: { jkt } }, issuerPrivateKey)
return token
```

#### 🟦 검증자(시그널링) 담당

- **핸드셰이크 프레임 파싱 (순수 TCP)** - 데이터 중계는 HTTP 헤더가 없으므로, 연결 직후 최초 HELLO 프레임에서 `access_token`·`dpop_proof`를 추출하는 커넥션 게이트. 둘 중 하나라도 없으면 스트림을 열기 전에 즉시 거절. (시그널링이 WebSocket이면 upgrade 헤더에서 추출 병행)
- **CPU 인메모리 DPoP 검증 + 크로스 체크** - 외부 서버 질의 없이 로컬 연산으로 6단계의 3중 검증 수행. `jkt` 매칭이 핵심. **room id만으로는 절대 통과시키지 않는다(소유 증명 필수).**
- **`jti` 재사용 방지 (검증자 전담)** - 복제 리플레이 방어는 검증자만 할 수 있다. `jti`를 **인메모리에 기록·조회·만료**하며 생명주기를 관리한다. TTL은 `iat` 허용 시간창(예: ±60초)과 일치. 다중 인스턴스면 **짧은 TTL 공유 캐시(Redis 등)·sticky routing**으로 인스턴스 간 우회를 차단.
- **(선택) 서버 Nonce 발급** - 고위험 경로엔 `DPoP-Nonce`(핸드셰이크 응답 프레임에 실어)를 challenge로 내려 Proof에 포함시켜, 시간창·캐시 의존 없이 복제를 봉쇄.

```text
// 검증자 pseudo - TCP 핸드셰이크 게이트
frame = readHelloFrame(conn)              // 순수 TCP: HTTP 헤더가 아니라 프레임
at  = frame.access_token
prf = frame.dpop_proof
if (!at || !prf) reject()
if (!verify(at, issuerPubKey))                 reject("bad token")      // ①
if (!verify(prf, prf.header.jwk))              reject("bad proof")      // ②
if (sha256thumb(prf.header.jwk) != at.cnf.jkt) reject("key mismatch")   // ③ 소유==토큰주인
if (prf.htu != thisChannelId || prf.htm != "CONNECT") reject()          // 채널 매핑 대조
if (seenRecently(prf.jti))                     reject("replay")
cache(prf.jti, ttl=120s);  accept()       // 통과 후에야 스트림 개시
```

#### 🟩 클라이언트 (뷰어 · 호스트/에이전트 · 프론트) 담당

- **런타임 EC P-256 키쌍 생성 + 안전 보관** - 세션 시작 시 키쌍 생성. 개인키는 **메모리/보안 저장소**에만(브라우저는 `CryptoKey extractable:false`, 네이티브는 Keychain/Keystore). 로그·전송·디스크 평문 저장 금지.
- **연결(요청) 시점마다 Proof JWT 동적 서명** - 연결 직전 `htm`(순수 TCP는 `"CONNECT"` 고정)·`htu`(대상 채널·세션 식별자)·`iat`(현재 시각)·`jti`(매번 새 난수)를 채워 개인키로 서명. **재사용 금지 - 연결마다 새로 만든다.**
- **파트별 스택** - 프론트/뷰어(JS): WebCrypto `subtle.generateKey/sign`. 호스트·에이전트(네이티브): 플랫폼 crypto + OS 보안 저장소.

```text
// Client pseudo - 매 요청
proof = sign(
  { typ:"dpop+jwt", alg:"ES256", jwk: publicJwk },       // header
  { htm: method, htu: url, iat: now(), jti: randomId() }, // payload
  privateKey                                             // 은닉된 개인키
)
send(url, headers={ Authorization:"DPoP "+accessToken, DPoP: proof })
```

### 평가 요약

| 항목     | 평가                                                                               |
|--------|----------------------------------------------------------------------------------|
| 구현 난이도 | 🟡 중간~높음 - 3개 파트(발급 서버·검증자·클라이언트) 동시 작업 필요                                     |
| 강점     | 토큰만 탈취해도 **개인키 없으면 무용지물**. IP 변동 무관(오탐 없음). 표준(RFC 9449). 검증은 CPU 로컬 연산이라 무상태 유지 |
| 약점     | 3개 파트 동시 개발 부담. `jti` 재사용 방지용 **단기 캐시** 필요(단, 짧은 TTL 로컬로 충분)                     |
| 적합 상황  | 탈취 리스크가 실제 위협이고 클라이언트를 커스텀 제어 가능한 경우(에이전트·런처·뷰어). §14 P2로 단계 도입                  |

## 12. 두 방안 비교

| 기준        | A. 단말 정보 바인딩            | B. DPoP (소유 증명)      |
|-----------|-------------------------|----------------------|
| 묶는 대상     | IP·MAC·Device ID (속성 값) | 클라이언트 개인키 (암호학적 소유)  |
| 탈취 내성     | 단말 정보 함께 위조되면 뚫림        | 개인키 없으면 재사용 불가 🟢 강함 |
| 오탐(정상 차단) | NAT·모바일 IP 변동 시 발생      | 거의 없음                |
| 검증자 저장소   | 불필요 (stateless 유지)      | jti 재사용 방지용 단기 캐시    |
| 클라이언트 변경  | 헤더 추가 정도                | 키 생성·요청별 서명 로직 필요    |
| 구현 난이도    | 🟢 낮음                   | 🟡 중간~높음             |
| 표준화       | 커스텀                     | RFC 9449 표준          |

> 💡 **권장:** 단계적 도입. **1차로 exp(만료) 도입 + 방안 A(바인딩)**로 표면을 즉시 좁히고, 탈취 리스크가 핵심 위협인 경로(에이전트·런처)에 **2차로 방안 B(DPoP)**를 적용한다. 둘은 배타적이지 않고 병용 가능하다.

---

# Part IV · 설계 검토 및 결론

## 13. 키 롤링 + 배포 전략

키 롤링(§8 문제)은 JWKS로 신·구 키를 함께 노출하고, 검증자가 토큰 Header의 `kid`로 맞는 키를 고르게 하면 무중단으로 가능합니다. 갱신 전파 방식은 두 가지입니다.

| 방식             | 장점                     | 단점                |
|----------------|------------------------|-------------------|
| JWKS 폴링 (1~5분) | 단순·신뢰성 높음·디버깅 쉬움       | 전파 지연, 폴링 트래픽     |
| pub/sub 이벤트    | 실시간·효율적(다수 검증자에 유리)    | 복잡도·이벤트 유실/순서 리스크 |

> 💡 **권장:** Phase 1은 **JWKS 폴링 + 로컬 캐시**로 시작(과잉설계 회피). 키 교체가 빈번해지거나 자동 롤링(예: 정기 롤링)을 도입하면 **pub/sub으로 마이그레이션**. 아키텍처에 이벤트 버스 예약 공간만 남겨둔다.

## 14. 개선 과제 (Action Items)

즉시 착수할 실무 과제를 우선순위와 함께 정리한다. **두 축은 직교하며 어느 하나도 생략 불가다 - (인프라) 전송 기밀성·무결성(TLS)과 (애플리케이션) 소유권·인가(PoP·토픽 authz)는 서로 다른 구멍을 막으므로 반드시 병행한다(§8 직교성 표).** 근본 원인은 후자(인가 부재)이지만, 전자 없이는 스트림이 평문으로 노출된다.

### [인프라 / 네트워크 파트] - 노출 경로 축소 (필요조건)

> 💡 **타겟 정조준:** 시그널링 서버는 이미 `wss://`로 보호되므로 **대상이 아니다.** P0의 타겟은 **데이터 중계 서버 ↔ 클라이언트 구간의 평문**이다. 단, 이것은 **스니핑 경로를 닫는 필요조건**일 뿐 사고의 근본 원인(인가)은 P1에서 해결한다.

| 우선순위       | 과제                                                           | 세부                                                                                                                                                                                      |
|------------|--------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| 🔴 P0 · 즉시 | 데이터 중계 서버 접속 프로토콜 **`tcp://` → `ssl://`(TLS) 전면 교체**         | 데이터 중계 구간을 **TCP over TLS로 강제**(`ssl://` / `tls://`). 평문 `tcp://` 포트를 전수 점검 후 **차단·폐기**. **스니핑이라는 위험 지점을 없애지만, TURN 응답·MQTT 감청 경로로 새는 room id는 못 막으므로 P1과 반드시 병행.**            |
| 🟢 유지      | 시그널링 서버 `wss://` 유지                                          | 이미 안전 - 변경 불필요.                                                                                                                                                                       |

### [애플리케이션 / 백엔드 파트] - 근본 원인(인가) 제거

> 💡 **여기가 사고의 뿌리다.** room id가 "소지 = 접근"인 Bearer capability로 동작한 것이 근본 원인이므로, **소유 증명(PoP) + 토픽 권한 검증**으로 인가 모델 자체를 바로잡는다. P0(TLS)와 **병행**하며, "나중에 검토"가 아니다.

| 우선순위          | 과제                               | 세부                                                                                                   |
|---------------|----------------------------------|------------------------------------------------------------------------------------------------------|
| 🔴 P1 · 근본해결 | **소유 증명(PoP/DPoP) 도입** - 세션을 키 소유에 결착 | 클라이언트 임시 비대칭키 + `cnf` 지문 박제(§11). **room id를 어떤 경로로 얻어도 개인키 없이는 중계 서버 접속 거부.** 데이터 중계는 순수 TCP이므로 헤더가 아니라 **연결 핸드셰이크 프레임**에 토큰·Proof를 담아 검증(§11 5단계). Bearer 인가 모델을 근본에서 제거. |
| 🔴 P1 · 근본해결 | **MQTT 토픽 구독 권한 검증**             | 발급 토큰의 `authority`(sub/pub 토픽) 클레임을 브로커/검증자가 강제해 **자기 토픽만 구독 허용**. 남의 토픽 감청 경로(②)를 차단.                |
| 🟡 P1 · 병행    | 모든 발급 토큰에 **짧은 만료(`exp`) 강제**    | 예외 없이 `exp` 부여. 탈취·유출되어도 유효 창을 좁힌다. 만료 시 재발급 흐름 정비.                                                    |
| 🟡 P2 · 보강    | **단말 고유 정보 바인딩** (Claim 추가) + 키 롤링(JWKS 복수 키) + `aud`·`jti` 검증 | IP·MAC·Device ID를 `bnd` 클레임에 삽입해 저비용으로 표면 축소(§10, NAT·모바일 오탐 정책 병행). JWKS 신·구 키 병존 무중단 회전(§13), `aud`·`jti` 검증으로 표준 이탈 해소. |

> ⚠️ **착수 순서 요약:** **P0(데이터 중계 TLS)와 P1(PoP 소유 증명 + MQTT 토픽 authz + exp)을 병행**하라. TLS는 노출 경로를 줄이는 필요조건, **PoP·토픽 authz는 근본 원인(인가)을 제거하는 핵심 처방**이다. 단말 바인딩·키 롤링은 그 위의 보강(P2).

## 15. 최종 결론

> ✅
> 1. **현재 발급 서버 발급 / 검증자 로컬 검증 구조는 바꿀 필요가 없다 - OAuth2/OIDC의 정석이다.** 비대칭 JWS + JWKS + `kid`는 이종 서버 간 실시간 검증을 없애고 대규모 트래픽을 무상태·초고속으로 처리하기 위한 표준 설계다. 부족한 것은 구조가 아니라 **인가 설계와 운영 보안 디테일**이다.
> 2. **근본 원인은 스니핑이 아니라 인증/인가 부재(Broken Access Control)다.** room id(세션 ID)가 "소지 = 접근"인 Bearer capability로 동작해, TURN 응답·MQTT 감청 등으로 room id를 얻은 누구나 소유자 확인 없이 남의 세션에 붙을 수 있었다. **이것이 사고의 뿌리다.**
> 3. **근본 해결책은 소유 증명(PoP/DPoP) + MQTT 토픽 권한 검증이다.** 세션을 클라이언트 키 소유에 결착하면 room id를 얻어도 개인키 없이는 접속이 거부된다. 토픽 authz로 감청 경로도 닫는다. 데이터 중계는 순수 TCP이므로 HTTP 헤더가 아니라 **연결 핸드셰이크 프레임**에 토큰·Proof를 담아 검증한다.
> 4. **TLS와 PoP는 직교하므로 반드시 병행한다 - 어느 하나도 생략 불가.** TLS만 하면 정상 경로(TURN·MQTT)로 새는 room id 재사용을 못 막고(인가 구멍), PoP만 하면 접속은 막아도 **평문 스트림 도청·변조**를 못 막는다(기밀성·무결성 구멍, DPoP는 스트림 본문 미서명). TLS=전송 보호, PoP=접속 소유권 — 서로 다른 구멍을 막는다(§8 직교성 표).
> 5. **리플레이·표면 축소 보강**: 모든 토큰 `exp` 강제, 단말 바인딩, `aud`·`jti` 검증, JWKS 키 롤링(폴링→필요 시 pub/sub).
> 6. **검증자 세션 인증(발급 주체 이전)**은 아키텍처적으로 타당하나 저장소·커스텀 브로커 비용이 커, 현 시점에서는 위 처방으로 방어를 보강하는 편이 현실적이다.

---

*비(非)HTTP 실시간 릴레이 인증 아키텍처 보안 개선 및 토큰 표준 가이드 · 범용판*

- 구조(발급 서버 발급 / 검증자 무상태 검증)는 OAuth2 정석이다.
- **근본 원인은 인증/인가 부재** - room id만으로 소유자 확인 없이 세션 접속이 가능했다. 스니핑(평문 TCP)은 room id를 얻는 **위험 지점 중 하나**일 뿐 뿌리가 아니다.
- 방어는 **(필요조건) 데이터 중계 TLS + (근본 해결) 소유 증명(PoP)·MQTT 토픽 authz를 병행**하고, exp·바인딩·키 롤링으로 보강한다. 데이터 중계는 순수 TCP이므로 토큰·Proof는 **핸드셰이크 프레임**에 싣는다.

## Reference

- [RFC 9449 — OAuth 2.0 Demonstrating Proof of Possession (DPoP)](https://datatracker.ietf.org/doc/html/rfc9449)
- [RFC 7638 — JSON Web Key (JWK) Thumbprint](https://datatracker.ietf.org/doc/html/rfc7638)
- [RFC 7517 — JSON Web Key (JWK)](https://datatracker.ietf.org/doc/html/rfc7517)
- [IBM Docs — Demonstrating Proof of Possession](https://www.ibm.com/docs/ko/security-verify?topic=connect-demonstrating-proof-possession)

*관련 문서: [DPoP / PoP 정공법 아키텍처](dpop-pop-architecture.md) · [OpenID Connect(OIDC) 아키텍처 완전 정복](oidc-architecture.md)*
