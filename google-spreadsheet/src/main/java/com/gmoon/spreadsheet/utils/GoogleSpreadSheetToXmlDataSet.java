...
public class GoogleSpreadSheetToXmlDataSet {
  ...
  private static StringBuffer getValueElementStringBuffer(TableNode.RowNode rowNode) {
    ...
    for (TableNode.RowNode.ValueNode valueNode : rowNode.getValueElements()) {
      ...
      if (StringUtils.isBlank(columnValue)) {
        ...
        sb.append(String.format("\n         <value description=\"%s\">%s</value>", columnName, StringEscapeUtils.escapeXml(columnValue)));
      }
    }
    return sb;
  }
  ...
  private static TableNode.RowNode getRowNode(List<String> columnElements, List<CellData> cellDataList, boolean ignoreTestData) {
    ...
    for (int i = 0; i < columnElements.size(); i++) {
      ...
      try {
        ...
        if (i == 0) {
          ...
          if (ignoreTestData && !isDefaultData) break;
