...
| `PowerShell`           | Windows에서 PowerShell 명령 실행. opt-in 프리뷰 기능                                                             | Y                   |         |            | `CLAUDE_CODE_USE_POWERSHELL_TOOL=1` 환경변수로 활성화. Windows 전용 |
| `Read`                 | 파일 내용 읽기                                                                                                    | N                   |         |            |           |
| `ReadMcpResourceTool`  | URI로 특정 MCP 리소스 읽기                                                                                        | N                   |         |            |           |
| `SendMessage`          | 에이전트 팀 팀원에게 메시지 전송 또는 에이전트 ID로 서브에이전트 재개                                            | N                   |         |            | `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` 또는 `--agent-teams` 플래그 필요 |
| `Skill`                | 메인 대화 내에서 스킬 실행                                                                                        | Y                   |         |            |           |
| `TaskCreate`           | 작업 목록에 새 작업 생성                                                                                          | N                   |         |            |           |
| `TaskGet`              | 특정 작업의 전체 세부 정보 조회                                                                                   | N                   |         |            |           |
| `TaskList`             | 모든 작업의 현재 상태 목록 조회                                                                                   | N                   |         |            |           |
| ~~`TaskOutput`~~       | 백그라운드 작업의 출력 조회                                                                                       | N                   |         | deprecated (last-updated: 2026-04-02) | 대신 `Read` 도구에서 작업 출력 파일 경로를 사용 권장 |
| `TaskStop`             | ID로 실행 중인 백그라운드 작업 종료                                                                               | N                   |         |            |           |
| `TaskUpdate`           | 작업 상태, 의존성, 세부 정보 업데이트 또는 작업 삭제                                                             | N                   |         |            |           |
| `TeamCreate`           | 여러 팀원으로 에이전트 팀 생성                                                                                    | N                   |         |            | `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` 또는 `--agent-teams` 플래그 필요 |
| `TeamDelete`           | 에이전트 팀 해산 및 팀원 프로세스 정리                                                                            | N                   |         |            | `CLAUDE_CODE_EXPERIMENTAL_AGENT_TEAMS=1` 또는 `--agent-teams` 플래그 필요 |
...
- `PowerShell` 도구는 opt-in 프리뷰로, 프리뷰 기간 중 auto mode 미지원, 프로파일 미로드, 샌드박싱 미지원, WSL 미지원 등의 제한이 있습니다.
- `SendMessage`, `TeamCreate`, `TeamDelete`는 experimental agent teams 기능 활성화 시에만 사용 가능합니다.

## Reference

- [Claude Code - Tools reference](https://code.claude.com/docs/en/tools-reference)
- [Claude Code Release Notes](https://api.github.com/repos/anthropics/claude-code/releases/latest)
