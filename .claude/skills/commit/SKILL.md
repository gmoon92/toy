...
커밋을 실행하기 전에 계획을 아래 형식으로 출력하고, `AskUserQuestion` 툴로 확인을 받습니다.
...
**1/N** `feat(auth)`
파일: AuthService.ts, AuthController.ts
> feat(auth): 인증 로직 서비스 레이어로 분리
> - UserController의 인증 관련 코드 추출

**2/N** `docs(commit)`
파일: .claude/skills/commit/SKILL.md
> docs(commit): 커밋 스킬 문서 개선
```

계획을 출력한 직후 `AskUserQuestion` 툴을 호출합니다. 아래 파라미터를 그대로 사용하세요:

- **question**: `"커밋 계획대로 진행할까요?"`
- **header**: `"커밋 확인"`
- **options**:
  - label `"진행 (Recommended)"`, description `"위 계획대로 순서대로 커밋합니다"`
  - label `"수정"`, description `"메시지나 파일 그룹을 바꾸고 싶을 때"`
  - label `"취소"`, description `"커밋을 중단합니다"`

**선택 결과에 따른 처리:**

- **진행**: 5단계로 이동하여 순서대로 커밋 실행
- **수정**: `"어느 커밋을 어떻게 수정할까요? (예: 1번 메시지 변경, 2번과 3번 그룹 합치기)"`를 텍스트로 안내 → 사용자 입력을 반영해 계획 재출력 → `AskUserQuestion` 재호출
- **취소**: 커밋 중단 후 종료
