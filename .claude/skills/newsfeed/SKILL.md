---
name: claudecode-document-validator
description: |
...
- **필수 섹션 존재 여부 확인**: 문서 유형별 필수 섹션 검사 `${CLAUDE_SKILL_DIR}/references/checklists/{doc-type}.md` 활용
- **고정 템플릿 준수 여부 확인**: 검토/평가 에이전트 템플릿 검증 `${CLAUDE_SKILL_DIR}/references/templates/output.md` 활용

## Step 2: 프론트 매터 검증

`${CLAUDE_SKILL_DIR}/references/frontmatter/{doc-type}.md` 문서를 로드한다.

문서에 정의된 프론트 메터 기준으로 다음 세부 검증을 진행한다.

[ ] YAML 문법 오류 검증
[ ] 필수 필드 존재 여부 검증
[ ] 필드 타입 및 값 범위 검증
[ ] 지원하지 않는 필드 검증
[ ] `skills`, `agents`, `subagents` 필드 순한 참조 방지
[ ] 에이전트, 스킬 이름 일치 검증
[ ] 유니크한 `name` 일치 검증

## Step 3: 본문 구조 검증

`${CLAUDE_SKILL_DIR}/references/checklists/{doc-type}.md` 문서를 로드한다.

1. 마크다운 헤딩 레벨 검사
2. 필수 섹션 존재 확인
3. 링크/참조 유효성 검사
4. **일관성 및 가독성 평가**: 문서 품질 평가 및 개선 제안
5. **프론트 매터 전환 제안**: 본문의 에이전트 호출을 `subagents`/`agent` 필드로 전환 가능한지 제안
6. 본문-프론트매터 일치성: 본문의 에이전트 언급이 프론트 매터 `subagents`에 정의되어 있는지 확인
7. MCP 도구 이름: 정규화된 이름(`ServerName:tool_name`) 사용 여부 확인
- 중복 내용 체크: 모듈 CLAUDE.md와 루트 CLAUDE.md 간 중복 섹션 확인
...
1. 문서 유형에 따른 추가 규칙 적용
2. 크로스 레퍼런스 검증
3. 프론트메터 `subagents`, `skills` 필드 값에 정의된 에이전트, 스킬 이름은 실제 문서의 프론트메터의 `name` 필드 이름과 동일해야 한다.

**CLAUDE.md 특수 검증**:
- 루트 vs 모듈 CLAUDE.md 중복 내용 체크
- 명령어 예시가 실제 동작하는지 검토
- 문서 길이가 150줄 이하인지 확인
- 불필요한 섹션(Agent Guide 등)이 루트에 남아있는지 확인

## Step 5: 본문-프론트매터 일치성 검증

1. 본문에서 에이전트/스킬 호출 패턴 탐지
2. 프론트 매터로 정의 가능한 항목 식별
3. 변환 제안 생성

**검증 패턴:**
| 본문 패턴 | 프론트 매터 변환 권장 | 대상 |
|:----------|:---------------------|:-----|
...
2. 수정 제안 제공
3. 피드백 문서 저장 (`.claude/.tmp/validation/{CLAUDE_SESSION_ID`)
...
검증 결과는 `.claude/.tmp/claudecode-document-validator/${CLAUDE_SESSION_ID}` 디렉토리에 마크다운 파일로 저장됩니다.
...
```
.claude/.tmp/claudecode-document-validator/
