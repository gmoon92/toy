...
- 톰캣에서 `static` 변수 잘못 쓰면 메모리 누수가 생긴다던데, 진짜야?

이 문서는 그런 실전적인 궁금증에서 출발했다.
개인적으로는 `매직 상수`를 `static final`로 정의하는 과정에서 이 질문이 생겼다. 정적으로 선언하면 안전할 것 같았지만, 이 값들이 GC 대상인지,
또 서버 환경에서 의도치 않게 살아남아 메모리를 점유할 수 있는지 확신이 서지 않았다.

그래서 자연스럽게 JVM 메모리 구조, GC 대상 조건, 그리고 클래스 로더의 수명까지 파고들게 되었다.

단순한 이론 정리가 아니라,

**실전에서 “static을 어디까지 허용할 수 있을지” 기준을 세우기 위한 목적이었다.**
...
자바(JVM)가 관리하는 주요 메모리 영역은 다음과 같다.

| 영역                                       | 특징/내용                                         | 참고                                 |
|------------------------------------------|-----------------------------------------------|------------------------------------|
| 힙(Heap)                                  | new로 생성되는 객체와 배열 저장, GC의 주요 대상                | GC 대상 (참조 해제 시)                    |
| 스택(Stack)                                | 각 스레드별 메서드 호출 정보, 지역 변수, 기본형 변수 저장            | 메서드 호출 끝나면 소멸                      |
| 메서드 영역(Method Area) or 메타스페이스(Metaspace) | 클래스 정보, static 변수, static 메서드, 런타임 상수 풀 등 저장  | JDK7까지는 PermGen, JDK8부터는 Metaspace |
| 런타임 상수 풀(Runtime Constant Pool)          | 클래스별 상수(문자열, static final 등), String 리터럴 등 저장 | 메서드영역/메타스페이스 내부                    |

> **TIP:**  
> JDK7까지는 "메서드 영역" 내 Permanent Generation(=PermGen)  
> JDK8 이후에는 "Metaspace"가 사용된다.
